...
4. Open the versioned
   [`study.html?package=0.8.6-q6`](https://sasoup-yr.github.io/accessible-nasa-tlx-demo/study.html?package=0.8.6-q6)
...
`0.8.6-q6`. If generated JavaScript shows an earlier value, close that stale tab and
...
- the status changes from `Connecting the questionnaire` to `The questionnaire is
  connected`, names bridge `0.8.6-q6` and says the diagnostic fields were staged;
...
`__js_AQP_BRIDGE_BUILD = 0.8.6-q6`. `__js_AQP_ACCEPTED` is left unset until a
...
listener exists. Bridge `0.8.6-q6` uses a two-way ready handshake with an exact
