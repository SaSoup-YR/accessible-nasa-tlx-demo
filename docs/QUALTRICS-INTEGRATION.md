...
The four repository files are not Qualtrics uploads. They are separate installation
inputs:

| Content | Exact Qualtrics location | Do not |
| --- | --- | --- |
| Complete question HTML generated by `study.html` | One Text/Graphic question, opened in HTML/source view | Do not paste the static template unchanged or paste HTML into the ordinary rich-text view |
| `embedded-data-fields.txt` | One Embedded Data element near the start of Survey Flow; one non-empty line per field; values unset | Do not paste this list into a question |
| `qualtrics-question.js` | JavaScript editor for the same Text/Graphic question | Do not paste it into the question HTML or add `script` tags |
| `end-of-survey-message.txt` | Custom End of Survey message | Do not configure a redirect or paste it into the NASA-TLX question |

The generated installation package on `study.html` now contains four labelled text
boxes and four copy buttons so the static repository files do not need to be opened
or uploaded during setup.
...
6. In the Qualtrics question's HTML/source view, replace the entire question body with
   the **Complete question HTML** copied from the generated installation package. It
   contains a concrete configured participant URL, the live iframe and the conditional
   recorded-response/PDF summary. Do not paste the static
   [`integrations/qualtrics/question-html-template.html`](../integrations/qualtrics/question-html-template.html)
   unchanged because it deliberately contains a placeholder URL.
...
### What the Qualtrics editor should show

Qualtrics may display Embedded Data piped-text tokens such as
`${e://Field/__js_ANTLX_PARTICIPANT_CODE}` literally in the survey editing canvas.
That is not a participant test and does not mean that a value has been collected.

Use **Preview survey** after saving the complete generated HTML and JavaScript. Before
the response is recorded:

- the read-only recorded-response summary must be hidden;
- the configured GitHub participant questionnaire must be visible inside the iframe;
- the collection status must state that the questionnaire will save into the current
  Qualtrics response.

If Preview shows the raw summary instead of the iframe, clear the Text/Graphic question
and repeat step 6 in HTML/source view. If it shows a blank iframe, regenerate and copy
the complete question HTML from `study.html`; a static template with
`PASTE_THE_GENERATED_PARTICIPANT_PAGE_URL_HERE` cannot load the questionnaire.
