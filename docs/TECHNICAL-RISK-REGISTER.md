...
| R21 | A rendered iframe is mistaken for proof that `AQP_*` fields will be recorded | The bridge is hidden until the exact-origin child handshake succeeds, stages visible build diagnostics and the conductor requires a newly dated synthetic row; old rows are explicitly excluded from the check | Bridge, generated-package and migration tests | Confirm a new UCL row has `AQP_ACCEPTED=1`, schema 4 and the expected instrument ID |
