...
| R6 | Spoken landmark language changes a rating's meaning | Visible labels map to 0, 25, 50, 75 and 100; conflicts, negation and invalid values are rejected; confirmation is mandatory | Parser and component regression tests | Microphone test in each supported browser and accent context |
| R7 | The browser speech service is unavailable or transcribes inaccurately | Voice is optional; visible native controls and system voice control remain available; the transcript and proposed answer are shown and announced before confirmation | Component tests | Record browser/device support and observed transcripts |
