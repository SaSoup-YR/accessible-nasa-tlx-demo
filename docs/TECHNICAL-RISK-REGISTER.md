...
| R11 | Keyboard, screen reader, reflow, selection or focus behaves differently inside the Qualtrics iframe | Native controls, focus movement, status regions and exact iframe title are implemented; selected states persist with a check/label; control, selected, focus and gaze indicators use calculated ≥3:1 authored contrast | Axe structural scans, interaction tests and calculated contrast audit | NVDA/VoiceOver, forced colours, 200% zoom, 320 CSS-pixel reflow and mobile UCL-preview tests |
...
| R15 | Re-entering a participant code after a reload increases memory and task burden, or retaining it creates a shared-device privacy risk | When recovery is enabled, a valid pseudonymous code is held only in tab-scoped session storage; same-tab reload restores the code and focuses the saved-session offer; long-term local participant-code storage is not used | Component reload and focus test | Re-test browser session restore, shared-device procedure and participant wording |
