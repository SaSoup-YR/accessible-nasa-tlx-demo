...
Current checkpoint: **39 tests pass across nine files.**
...
- **standalone.test.ts** — Version 0.5 single-document structure, no Vite asset dependency, executable inline JavaScript syntax and compiled-component boot rendering.
- **component.test.ts** — ratings-before-pairs flow, pair-task clarity, definition non-duplication, adjustable text, one primary Smiley presentation, precision refinement, interruption recovery, audible guidance, confirmed voice, non-duplicated review numbering and confirmed gaze entry.
...
- **study-component.test.ts** — configured participant-code gate, locked prepared support, full completion saving, score-display policy and host-event emission.
- **accessibility.test.ts** — axe-core structural scans of the introduction, a supported rating screen and the study-conductor setup page.
