...
Current checkpoint: **62 tests pass across eleven files.**
...
- **result-sink.test.ts** — explicit host installation, same-submission receipt validation, exact-origin Qualtrics messaging, direct-opening rejection and false-success rejection.
...
- **conductor-component.test.ts** — researcher/participant separation, participant-choice default, local and origin-bound Qualtrics configuration, configuration/result file distinction and conductor error focus.
- **study.test.ts** — validated UTF-8 configuration links, identifier rejection, complete pseudonymous record storage, duplicate protection and stable CSV/support-change fields.
- **study-component.test.ts** — configured participant-code gate, locked, presentation-only and participant-choice policies, support-change provenance, configured-task spoken summary, local completion, approved-host receipt, host failure retention, score-display policy and host-event emission.
