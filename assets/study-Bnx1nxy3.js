import{w as Be,g as Re,x as Ge,M as Se,a as v,t as Je,i as He,D as Ve,m as re,y as We,z as ze,u as Ye,o as Xe,B as Ze,h as Ke,C as Ae,P as ue,A as Q,l as m,k as ce,j as et,E as tt,F as nt,f as Ie}from"./shared-BRA3mK6s.js";const de=`__js_AQP_ACCEPTED
...
`,j=20;let Te=0;function se(e={}){return Te+=1,{key:`custom-item-${Te}`,name:"",prompt:"",lowAnchor:"",highAnchor:"",simpleExplanation:"",reverseScored:!1,...e}}function qe(){return{name:"",shortName:"",version:"1.0.0",description:"A researcher-supplied questionnaire.",introPrompt:"Answer each item about the task that you have just completed.",sourceLabel:"Researcher-supplied questionnaire",sourceUrl:"",scaleType:"agreement",minimum:1,maximum:5,step:1,scoreName:"Questionnaire score",aggregation:"mean",items:[se({name:"Item 1"}),se({name:"Item 2"})]}}function R(e,t){const n=e.trim().replace(/\s+/g," ");if(!n)throw new Error(`${t} is required.`);return n}function st(e){return e.trim().toLowerCase().normalize("NFKD").replace(/[\u0300-\u036f]/g,"").replace(/[^a-z0-9]+/g,"-").replace(/^-+|-+$/g,"").slice(0,50)}function ot(e,t){const n=t.trim()||e.trim();return n.length<=240?n:`${n.slice(0,237).trimEnd()}...`}function $e(e){if(!Number.isInteger(e.minimum)||!Number.isInteger(e.maximum)||!Number.isInteger(e.step))throw new Error("Scale minimum, maximum and step must be whole numbers.");if(!Array.isArray(e.items)||e.items.length<1||e.items.length>j)throw new Error(`Add between 1 and ${j} questionnaire items.`);const t=R(e.name,"Questionnaire name"),n=R(e.shortName,"Short name"),r=st(n||t);if(!r)throw new Error("Questionnaire name must contain at least one Latin letter or number for its stable ID.");const i=e.items.map((f,l)=>{const c=R(f.prompt,`Item ${l+1} question`),p=f.simpleExplanation.trim().replace(/\s+/g," ");return{id:`item-${String(l+1).padStart(2,"0")}`,name:R(f.name,`Item ${l+1} label`),prompt:c,...p?{simpleExplanation:p}:{},shortMeaning:ot(c,p),lowAnchor:R(f.lowAnchor,`Item ${l+1} low endpoint`),highAnchor:R(f.highAnchor,`Item ${l+1} high endpoint`),...f.responseLabels?{responseLabels:f.responseLabels}:{}}}),o=e.items.flatMap((f,l)=>f.reverseScored?[`item-${String(l+1).padStart(2,"0")}`]:[]),u=i.every(f=>f.simpleExplanation),h=e.aggregation==="mean"?e.minimum:e.minimum*i.length,a=e.aggregation==="mean"?e.maximum:e.maximum*i.length,w={schemaVersion:1,id:`custom-${r}`,version:R(e.version,"Questionnaire version"),name:t,shortName:n,description:R(e.description,"Questionnaire description"),introPrompt:R(e.introPrompt,"Participant instruction"),officialContentNotice:"This questionnaire definition was supplied by the study conductor. Its wording, use and interpretation must match the approved study protocol.",source:{label:R(e.sourceLabel,"Source or authorship label"),...e.sourceUrl.trim()?{url:e.sourceUrl.trim()}:{}},scale:{type:e.scaleType,minimum:e.minimum,maximum:e.maximum,step:e.step},items:i,scoring:{strategy:e.aggregation==="mean"?"mean-v1":"sum-v1",scoreName:R(e.scoreName,"Score name"),minimum:h,maximum:a,...o.length?{reverseItemIds:o}:{}},supports:{simplerExplanations:u,smileyLandmarks:!1}};return Ne(w)}function Ne(e){const t=Be(e);if(Re(t.id))throw new Error("A custom questionnaire cannot replace a built-in questionnaire ID.");if(!t.id.startsWith("custom-"))throw new Error("A custom questionnaire ID must start with custom-.");if(t.scoring.strategy!=="mean-v1"&&t.scoring.strategy!=="sum-v1")throw new Error("A custom questionnaire must use the reviewed mean or sum scorer.");if(t.pairwise)throw new Error("Custom questionnaires do not support pairwise comparisons.");if(t.landmarks||t.supports.smileyLandmarks)throw new Error("Custom questionnaires do not support smiley landmarks.");const n=Ge(t);if(n>Se)throw new Error(`The questionnaire definition is ${n} bytes; the participant-link limit is ${Se} bytes.`);return t}function at(e){return`${e.id}-${e.version.replace(/[^A-Za-z0-9._-]+/g,"-")}.questionnaire.json`}const lt=2e6,me=/<\s*(?:script|iframe|object|embed|style)\b|(?:href|src)\s*=\s*["']?\s*javascript:|\son[a-z]+\s*=/i,he=/<\s*(?:img|picture|video|audio|canvas|svg|math|form|input|button|select|textarea|table)\b/i,fe=/\$\{(?:e|q|lm|gr)?:?\/?\/|q:\/\/|\{(?:if|TOKEN|INSERTANS|[A-Za-z][A-Za-z0-9_.]*\.(?:NAOK|shown))\b/i;function C(e){return e&&typeof e=="object"&&!Array.isArray(e)?e:null}function q(e){return typeof e=="string"||typeof e=="number"?String(e):""}function I(e){return e.replace(/\s+/g," ").trim()}function A(e,t){e.confirmationCodes.has(t.code)||(e.confirmationCodes.add(t.code),e.confirmations.push(t))}function U(e,t,n,r){const i=q(e);if(!i.trim())return r.unsupported.push({code:"missing-visible-text",title:`${t} is empty`,detail:`${n} does not contain participant-visible text.`}),null;if(me.test(i)||fe.test(i))return r.unsupported.push({code:"unsafe-dynamic-content",title:`${t} contains executable or dynamic content`,detail:`${n} contains script, an event handler, a JavaScript URL or survey-expression text. The importer does not execute or approximate it.`}),null;if(he.test(i))return r.unsupported.push({code:"unsupported-structured-content",title:`${t} contains media, a table or an interactive control`,detail:`${n} cannot be represented as one safe plain-text rating item without changing participant-visible content.`}),null;const u=new DOMParser().parseFromString(`<body>${i}</body>`,"text/html"),h=I(u.body.textContent??"");return h?((/<[^>]+>/.test(i)||h!==I(i))&&A(r,{code:"plain-text-normalisation",title:"Formatting was converted to plain text",detail:"Imported wording is rendered as safe plain text. Review it against the source export before use."}),h):(r.unsupported.push({code:"missing-visible-text",title:`${t} has no readable text`,detail:`${n} contains no participant-visible text after safe text extraction.`}),null)}function oe(e){const t=typeof e=="number"?e:Number(q(e).trim());return Number.isInteger(t)?t:null}function ge(e){if(e.length<2||e.some(n=>n<0||n>100)||new Set(e).size!==e.length)return!1;const t=e[1]-e[0];return t>0&&e.every((n,r)=>r===0||n-e[r-1]===t)}function De(e,t){if(!Array.isArray(e)||e.length!==Object.keys(t).length)return null;const n=e.map(String);return new Set(n).size===n.length&&n.every(r=>r in t)?n:null}function Le(e,t,n,r){const i=C(e);return U(i?.Display,t,n,r)}function ut(e){return e.flatMap(n=>n.labels).map(n=>n.toLowerCase()).some(n=>n.includes("agree")||n.includes("disagree"))?"agreement":"semantic-differential"}function ct(e){const t=e.split(/\s+/).map(r=>r.replace(/[^A-Za-z0-9]/g,"")[0]??"").join("").slice(0,12);return t?t.toUpperCase():e.replace(/[^A-Za-z0-9]+/g,"").slice(0,12)||"IMPORTED"}function ke(e,t,n,r,i){if(!r.length)return i.unsupported.push({code:"no-supported-items",title:"No supported questionnaire items were found",detail:"This release needs at least one required, ordered single-choice rating item."}),null;if(r.length>j)return i.unsupported.push({code:"too-many-items",title:"The questionnaire has too many items for a participant link",detail:`${r.length} supported items were found; this release accepts at most ${j}.`}),null;const o=r[0].values;if(!ge(o))return i.unsupported.push({code:"unsupported-response-values",title:"Response values are not one increasing whole-number scale",detail:"Values must be unique whole numbers from 0 to 100 with one constant positive step."}),null;const u=o.join("|");return r.some(h=>h.values.join("|")!==u)?(i.unsupported.push({code:"mixed-response-scales",title:"Items use different response values",detail:"The current participant interface requires every imported item to share the same ordered numeric scale."}),null):(A(i,{code:"review-scoring",title:"Scoring and reverse scoring require confirmation",detail:"Answer recodes do not establish a questionnaire score. Choose reviewed mean or sum, then mark any reverse-scored items."}),A(i,{code:"review-scale-type",title:"The scale description requires confirmation",detail:"Confirm whether the imported scale should be described as agreement or semantic differential."}),{name:e.slice(0,120),shortName:ct(e),version:"1.0.0",description:`Questionnaire imported from ${t}.`,introPrompt:n.slice(0,400),sourceLabel:t,sourceUrl:"",scaleType:ut(r),minimum:o[0],maximum:o.at(-1),step:o[1]-o[0],scoreName:"Questionnaire score",aggregation:"mean",items:r.map(h=>se({name:h.name.slice(0,120),prompt:h.prompt.slice(0,1e3),lowAnchor:h.labels[0].slice(0,80),highAnchor:h.labels.at(-1).slice(0,80),responseLabels:Object.fromEntries(h.values.map((a,w)=>[String(a),h.labels[w].slice(0,120)]))}))})}function X(e){return{code:`item-${e.sourceId}`,title:`${e.name}: imported`,detail:`${e.sourceId}; “${e.prompt}”; ordered choices: `+e.values.map((t,n)=>`${t} = ${e.labels[n]}`).join("; ")}}function ye(e,t,n,r,i,o,u={}){return{source:e,sourceName:t,fileName:n,title:r,draft:i,imported:o.imported,confirmations:o.confirmations,unsupported:o.unsupported,canConvert:!!i&&o.unsupported.length===0,...u}}function Oe(){return{imported:[],confirmations:[],unsupported:[],confirmationCodes:new Set}}function Ce(e,t){const n=C(e);if(!n)throw new Error(t);return n}function dt(e,t){const n=e.find(p=>p.Element==="BL"),r=Array.isArray(n?.Payload)?n.Payload.map(C).filter(p=>!!p):[],i=r.filter(p=>p.Type!=="Trash"),o=new Set(r.filter(p=>p.Type==="Trash").flatMap(p=>Array.isArray(p.BlockElements)?p.BlockElements:[]).map(C).filter(p=>!!p).map(p=>q(p.QuestionID)).filter(Boolean));o.size&&A(t,{code:"qualtrics-trash-ignored",title:"Questions in the Qualtrics Trash block were ignored",detail:`${o.size} deleted question${o.size===1?"":"s"} will not appear in the converted questionnaire.`});const u=e.find(p=>p.Element==="FL"),h=C(u?.Payload),a=Array.isArray(h?.Flow)?h.Flow.map(C).filter(p=>!!p):[];if(i.length!==1||a.length!==1||a[0].Type!=="Block")return t.unsupported.push({code:"qualtrics-complex-flow",title:"Qualtrics flow is outside the supported single-block subset",detail:"Use one ordinary question block only. Branches, randomisers, embedded-data flow and multiple blocks are not removed or flattened."}),{order:[],trashQuestionIds:o};const w=q(a[0].ID),f=i.find(p=>q(p.ID)===w);if(!f)return t.unsupported.push({code:"qualtrics-missing-block",title:"The active Qualtrics block could not be resolved",detail:`Survey Flow references ${w||"an unknown block"}.`}),{order:[],trashQuestionIds:o};const l=Array.isArray(f.BlockElements)?f.BlockElements:[],c=[];for(const p of l){const E=C(p);if(!E||E.Type!=="Question"||!q(E.QuestionID)){t.unsupported.push({code:"qualtrics-non-question-block-item",title:"The Qualtrics block contains unsupported content",detail:"Only ordered question entries are supported in the imported block."});continue}(E.SkipLogic||E.DisplayLogic)&&t.unsupported.push({code:"qualtrics-question-logic",title:`${q(E.QuestionID)} uses question logic`,detail:"Skip and display logic are not imported."}),c.push(q(E.QuestionID))}return{order:c,trashQuestionIds:o}}function pt(e){return["QuestionJS","JavaScript","DisplayLogic","SkipLogic","ChoiceDisplayLogic","CarryForward","Randomization","ChoiceRandomization"].some(t=>e[t]!==void 0&&e[t]!==null)}function Ee(e,t,n,r,i){const o=C(e[t]);if(!o)return i.unsupported.push({code:"qualtrics-missing-scale",title:`${r} has no answer scale`,detail:`${t} must contain every visible answer option.`}),null;const u=De(e[n],o);if(!u)return i.unsupported.push({code:"qualtrics-unknown-answer-order",title:`${r} has no reliable answer order`,detail:`${n} must list every ${t.toLowerCase()} entry exactly once.`}),null;const h=C(e.RecodeValues);let a;if(h){const f=u.map(l=>oe(h[l]));if(f.some(l=>l===null))return i.unsupported.push({code:"qualtrics-nonnumeric-recodes",title:`${r} has missing or non-integer recode values`,detail:"Every visible option needs an explicit whole-number recode."}),null;a=f}else{if(!u.every((l,c)=>l===String(c+1)))return i.unsupported.push({code:"qualtrics-missing-scale",title:`${r} has no explicit recode table or default sequential choice IDs`,detail:"Without RecodeValues, ChoiceOrder must explicitly list the default IDs 1 through N. The importer will not infer values from other object keys."}),null;a=u.map((l,c)=>c+1),A(i,{code:"qualtrics-default-recodes",title:"Qualtrics default sequential recodes were used",detail:"The QSF omits RecodeValues and explicitly orders default choice IDs 1 through N. Confirm these values against the source survey before conversion."})}const w=u.map((f,l)=>Le(o[f],`Answer ${l+1}`,`${r}/${f}`,i));return w.some(f=>f===null)?null:{values:a,labels:w}}function mt(e,t){let n;try{n=JSON.parse(e)}catch{throw new Error("The Qualtrics QSF file is not valid JSON.")}const r=Ce(n,"The Qualtrics QSF root must be a JSON object."),i=Ce(r.SurveyEntry,"The file does not contain a Qualtrics SurveyEntry.");if(!Array.isArray(r.SurveyElements))throw new Error("The file does not contain Qualtrics SurveyElements.");const o=Oe(),u=I(q(i.SurveyName))||"Imported Qualtrics questionnaire",h=r.SurveyElements.map(C).filter(d=>!!d),{order:a,trashQuestionIds:w}=dt(h,o),f=new Set(h.map(d=>q(d.Element)).filter(d=>d&&!["BL","FL","SQ"].includes(d)));f.size&&A(o,{code:"qualtrics-platform-settings-not-imported",title:"Qualtrics platform and presentation settings are not imported",detail:`The following non-question element types remain in Qualtrics only: ${[...f].sort().join(", ")}. Review the converted participant presentation.`});const l=new Map(h.filter(d=>d.Element==="SQ").map(d=>[q(d.PrimaryAttribute),C(d.Payload)]));if(a.length){const d=new Set(a);for(const S of l.keys())!S||d.has(S)||w.has(S)||o.unsupported.push({code:"qualtrics-unreferenced-question",title:`${S} is outside the imported active block`,detail:"The importer will not silently omit an active question that is not represented by the supported single-block flow."})}const c=[];for(const d of a){const S=l.get(d);if(!S){o.unsupported.push({code:"qualtrics-missing-question",title:`${d} is missing`,detail:"The active block references a question that is not present in SurveyElements."});continue}if(pt(S)){o.unsupported.push({code:"qualtrics-unsupported-behaviour",title:`${d} contains logic, randomisation or code`,detail:"The importer does not execute or remove question behaviour."});continue}const Z=C(S.Validation);if(C(Z?.Settings)?.ForceResponse!=="ON"){o.unsupported.push({code:"qualtrics-optional-question",title:`${d} is not a forced-response question`,detail:"The participant platform currently requires every imported item."});continue}const T=U(S.QuestionText,"Question text",d,o);if(!T)continue;const B=q(S.QuestionType),k=q(S.Selector),z=q(S.SubSelector),O=I(q(S.DataExportTag))||d;if(B==="MC"&&k==="SAVR"){const G=Ee(S,"Choices","ChoiceOrder",d,o);if(!G)continue;const N={sourceId:d,name:O,prompt:T,...G};c.push(N),o.imported.push(X(N));continue}if(B==="Matrix"&&k==="Likert"&&z==="SingleAnswer"){const G=Ee(S,"Answers","AnswerOrder",d,o),N=C(S.Choices),K=N?De(S.ChoiceOrder,N):null;if(!G||!N||!K){K||o.unsupported.push({code:"qualtrics-unknown-row-order",title:`${d} has no reliable matrix row order`,detail:"ChoiceOrder must list every matrix row exactly once."});continue}A(o,{code:"qualtrics-matrix-expanded",title:"Single-answer matrix rows were expanded into items",detail:"Review each generated item against the original matrix before conversion."}),K.forEach((D,ee)=>{const J=Le(N[D],`Matrix row ${ee+1}`,`${d}/${D}`,o);if(!J)return;const te={sourceId:`${d}/${D}`,name:J,prompt:`${T} — ${J}`,...G};c.push(te),o.imported.push(X(te))});continue}o.unsupported.push({code:"qualtrics-unsupported-question",title:`${d} uses an unsupported Qualtrics question type`,detail:`${B||"Unknown"} / ${k||"Unknown"} / ${z||"none"} is not converted. Supported: MC/SAVR and Matrix/Likert/SingleAnswer.`})}const E=(q(i.SurveyDescription).trim()?U(i.SurveyDescription,"Survey description","SurveyEntry",o):null)??"Answer each imported item about the task that you have just completed.",ae=ke(u,"Imported from Qualtrics QSF",E,c,o);return ye("qualtrics-qsf","Qualtrics QSF",t,u,ae,o)}function P(e,t){const n=[...e.documentElement.children].find(i=>i.localName===t);if(!n)return[];const r=[...n.children].find(i=>i.localName==="rows");return r?[...r.children].filter(i=>i.localName==="row").map(i=>Object.fromEntries([...i.children].map(o=>[o.localName,o.textContent??""]))):[]}function F(e,t,n,r,i){return e.find(o=>o[t]===n&&(!o.language||o.language===i))?.[r]??""}const ht={answer_width:["0"],answer_order:["normal"],array_filter_style:["0"],clear_default:["N"],hidden:["0"],hide_tip:["0"],max_answers:["1"],min_answers:["1"],other_comment_mandatory:["0"],other_numbers_only:["0"],other_position:["default"],page_break:["0"],public_statistics:["0"],random_order:["0"],save_as_default:["N"],scale_export:["0"],slider_rating:["0"],statistics_graphtype:["0"],statistics_showgraph:["1"],time_limit_action:["1"],time_limit_disable_next:["0"],time_limit_disable_prev:["0"],use_dropdown:["0"]};function ft(e,t){if(!e.length)return;const n=e.filter(r=>{const i=r.attribute?.trim()??"",o=r.value?.trim()??"";return o?!ht[i]?.includes(o):!1});if(n.length){const r=n.slice(0,6).map(i=>`${i.qid||"unknown question"}: ${i.attribute||"unknown"}=${i.value||""}`).join("; ");t.unsupported.push({code:"limesurvey-question-attributes",title:"The LimeSurvey export contains active or unknown question attributes",detail:`${r}${n.length>6?`; and ${n.length-6} more`:""}. These settings may change validation, randomisation, timing or presentation and are not discarded.`});return}A(t,{code:"limesurvey-default-question-attributes",title:"Default LimeSurvey question settings were not imported",detail:"Only empty or known default attributes were present; no active validation, randomisation or timing rule was found. Review the converted participant presentation."})}function Qe(e){if(!e.trim())return"";const n=new DOMParser().parseFromString(`<body>${e}</body>`,"text/html");return I(n.body.textContent??"")}function gt(e){const t=e.map(i=>oe(i.sortorder));if(t.some(i=>i===null))return!1;const n=t;if(new Set(n).size!==n.length)return!1;const r=n[0];return(r===0||r===1)&&n.every((i,o)=>i===r+o)}function yt(e){return e.every((t,n)=>t.code===`A${String(n+1).padStart(3,"0")}`)}function Pe(e,t,n,r,i){const o=e.qid||e.title||"unknown-question",u=t.filter(l=>l.qid===e.qid&&(!l.scale_id||l.scale_id==="0")).sort((l,c)=>(Number(l.sortorder)||0)-(Number(c.sortorder)||0));if(!gt(u))return i.unsupported.push({code:"limesurvey-answer-order",title:`${e.title||o} has no reliable answer order`,detail:"Every answer needs one unique consecutive sort order starting at 0 or 1."}),null;const h=u.map(l=>oe(l.code)),a=u.map(l=>oe(l.assessment_value));let w;if(h.every(l=>l!==null)&&ge(h))w=h;else if(a.every(l=>l!==null)&&ge(a))w=a,A(i,{code:"limesurvey-assessment-values",title:"LimeSurvey assessment values were used as response values",detail:"Verify every imported value against the LimeSurvey answer table."});else if(yt(u))w=u.map((l,c)=>c+1),A(i,{code:"limesurvey-positional-values",title:"LimeSurvey default answer codes were converted to ordered positions",detail:"The source uses A001 through A00N rather than numeric scores. The converted scale uses positions 1 through N. Confirm the intended values and scoring before conversion."});else return i.unsupported.push({code:"limesurvey-unsupported-values",title:`${e.title||o} has no safe increasing numeric recode`,detail:"Answer codes or assessment values must form one increasing whole-number scale with a constant step."}),null;const f=u.map((l,c)=>{const p=F(n,"aid",l.aid,"answer",r)||l.answer;return I(p??"")?U(p,`Answer ${c+1}`,`${e.title||o}/${l.code}`,i):(A(i,{code:"limesurvey-blank-scale-labels",title:"Blank scale positions will be shown as their numeric values",detail:"LimeSurvey leaves one or more intermediate labels blank. The converted questionnaire shows the reviewed numeric response value at those positions."}),String(w[c]))});return f.some(l=>!l)?null:{values:w,labels:f}}function vt(e,t,n){if(/<!DOCTYPE|<!ENTITY|<\?xml-stylesheet/i.test(e))throw new Error("The LimeSurvey file contains a DTD, entity or stylesheet declaration and was not parsed.");const i=new DOMParser().parseFromString(e,"application/xml");if(i.querySelector("parsererror"))throw new Error("The LimeSurvey file is not valid XML.");const o=i.querySelector("LimeSurveyDocType")?.textContent?.trim();if(o!=="Survey"&&o!=="Group")throw new Error("The XML file is not a LimeSurvey survey or question-group export.");const u=o==="Group"?"limesurvey-lsg":"limesurvey-lss",h=o==="Group"?"LimeSurvey LSG":"LimeSurvey LSS",a=Oe(),w=[...i.querySelectorAll("languages > language")].map(s=>s.textContent?.trim()??"").filter(Boolean),f=P(i,"surveys"),c=I(f[0]?.language??"")||w[0]||"";!c||w.length&&!w.includes(c)?a.unsupported.push({code:"limesurvey-base-language",title:"The LimeSurvey base language could not be resolved",detail:"The survey language must match one language declared in the LSS export."}):w.length>1&&A(a,{code:"limesurvey-base-language-only",title:`Only the LimeSurvey base language (${c}) will be imported`,detail:`Additional languages (${w.filter(s=>s!==c).join(", ")}) remain in LimeSurvey and are not included in the converted definition. Confirm the intended participant language.`});const p=P(i,"surveys_languagesettings"),E=p.find(s=>!s.surveyls_language||s.surveyls_language===c)??p[0],ae=I(E?.surveyls_title??"")||"Imported LimeSurvey questionnaire",d=P(i,"groups"),S=P(i,"group_l10ns"),Z=P(i,"questions");if(!d.length)throw new Error("The LimeSurvey export contains no questionnaire group.");const le=d.slice().sort((s,b)=>(Number(s.group_order)||0)-(Number(b.group_order)||0)).map(s=>{const b=S.find($=>$.gid===s.gid&&(!$.language||$.language===c)),Y=Z.filter($=>$.gid===s.gid&&(!$.parent_qid||$.parent_qid==="0"));return{id:s.gid,name:I(b?.group_name??s.group_name??"")||`Group ${s.gid}`,questionCount:Y.length,questionTypes:[...new Set(Y.map($=>$.type||"unknown"))]}});if(o==="Survey"&&d.length>1&&!n)return ye(u,h,t,ae,null,a,{requiresGroupSelection:!0,groupOptions:le});const T=n?d.find(s=>s.gid===n):d[0];if(!T)throw new Error("Choose a questionnaire group contained in this LimeSurvey export.");const B=S.find(s=>s.gid===T.gid&&(!s.language||s.language===c)),k=I(B?.group_name??T.group_name??"")||`Group ${T.gid}`,z=Z.filter(s=>s.gid===T.gid),O=new Set(z.map(s=>s.qid)),N=P(i,"subquestions").filter(s=>s.gid===T.gid||O.has(s.parent_qid));for(const s of N)O.add(s.qid);o==="Survey"&&d.length>1&&A(a,{code:"limesurvey-selected-group-only",title:`Only “${k}” will be converted`,detail:`${d.length-1} other survey group${d.length===2?"":"s"} will remain outside the converted questionnaire. The researcher selected this group explicitly; no other group is silently flattened or removed.`}),T.grelevance&&T.grelevance!=="1"&&A(a,{code:"limesurvey-group-relevance-not-retained",title:"The source group display condition will not be retained",detail:`The LimeSurvey group condition is “${I(T.grelevance)}”. The converted questionnaire runs this selected group as a standalone instrument. Confirm that this is intended.`}),I(T.randomization_group??"")&&A(a,{code:"limesurvey-group-randomisation-not-retained",title:"The source group randomisation setting will not be retained",detail:"The converted questionnaire uses the reviewed exported question order. Confirm that a fixed order is suitable."}),P(i,"conditions").filter(s=>O.has(s.qid)||O.has(s.cqid)).length&&a.unsupported.push({code:"limesurvey-conditions",title:"The selected LimeSurvey group contains question conditions",detail:"Branching and conditional relevance are not imported."});for(const[s,b]of[["assessments","assessment rules"],["defaultvalues","default answers"],["quotas","quota rules"],["quota_members","quota membership rules"],["quota_languages","localised quota messages"]])P(i,s).length&&a.unsupported.push({code:`limesurvey-${s.replace(/_/g,"-")}`,title:`The LimeSurvey export contains ${b}`,detail:`${b[0].toUpperCase()}${b.slice(1)} are not executed or silently discarded.`});ft(P(i,"question_attributes").filter(s=>O.has(s.qid)),a);const D=P(i,"question_l10ns"),ee=P(i,"answers"),J=P(i,"answer_l10ns"),te=z.filter(s=>!s.parent_qid||s.parent_qid==="0").sort((s,b)=>(Number(s.question_order)||0)-(Number(b.question_order)||0)),L=[];A(a,{code:"limesurvey-platform-settings-not-imported",title:"LimeSurvey platform and presentation settings are not imported",detail:"Theme, navigation, notification and publication settings remain in LimeSurvey. Review the converted participant presentation."});for(const s of te){const b=s.qid||s.title||"unknown-question";if(s.mandatory!=="Y"){a.unsupported.push({code:"limesurvey-optional-question",title:`${s.title||b} is not mandatory`,detail:"The participant platform currently requires every imported item."});continue}if(s.other&&s.other!=="N"){a.unsupported.push({code:"limesurvey-other-answer",title:`${s.title||b} allows an “Other” answer`,detail:"Free-text “Other” responses are not imported."});continue}if(s.relevance&&s.relevance!=="1"){a.unsupported.push({code:"limesurvey-question-relevance",title:`${s.title||b} uses relevance logic`,detail:"Conditional questions are not imported."});continue}const Y=F(D,"qid",s.qid,"question",c)||s.question,$=F(D,"qid",s.qid,"help",c)||s.help,je=F(D,"qid",s.qid,"script",c)||s.script;if(I(je||"")){a.unsupported.push({code:"limesurvey-question-script",title:`${s.title||b} contains a question script`,detail:"Imported scripts are never executed or silently removed."});continue}if(me.test($||"")||fe.test($||"")||he.test($||"")){a.unsupported.push({code:"limesurvey-question-help-structure",title:`${s.title||b} contains dynamic or structured help content`,detail:"Executable code, expressions, media and interactive help are not imported."});continue}if(Qe($||"")){a.unsupported.push({code:"limesurvey-question-help",title:`${s.title||b} contains participant help text`,detail:"Question help text is not silently merged into or removed from the item wording."});continue}const M=I(Y??"")?U(Y,"Question text",s.title||b,a):null;if(s.type==="5"){if(!M){a.unsupported.push({code:"missing-visible-text",title:"Question text is empty",detail:`${s.title||b} does not contain participant-visible text.`});continue}const x={sourceId:s.title||b,name:s.title||`Item ${L.length+1}`,prompt:M,values:[1,2,3,4,5],labels:["1","2","3","4","5"]};L.push(x),a.imported.push(X(x))}else if(s.type==="L"){if(!M){a.unsupported.push({code:"missing-visible-text",title:"Question text is empty",detail:`${s.title||b} does not contain participant-visible text.`});continue}const x=Pe(s,ee,J,c,a);if(!x)continue;const H={sourceId:s.title||b,name:s.title||`Item ${L.length+1}`,prompt:M,...x};L.push(H),a.imported.push(X(H))}else if(s.type==="F"){const x=N.filter(_=>_.parent_qid===s.qid).sort((_,V)=>(Number(_.question_order)||0)-(Number(V.question_order)||0));if(!x.length){a.unsupported.push({code:"limesurvey-array-without-rows",title:`${s.title||b} has no array row`,detail:"A LimeSurvey Array question must contain at least one explicit row before it can be converted."});continue}if(x.some(_=>_.relevance&&_.relevance!=="1"||_.other&&_.other!=="N"||_.scale_id&&_.scale_id!=="0")){a.unsupported.push({code:"limesurvey-array-row-behaviour",title:`${s.title||b} contains an unsupported array row`,detail:"Conditional, “Other” or secondary-scale array rows are not flattened."});continue}const H=Pe(s,ee,J,c,a);if(!H)continue;x.length===1?A(a,{code:"limesurvey-single-row-array-expanded",title:"Single-row LimeSurvey Array questions were converted to rating items",detail:"Each source question contains one array row. Review the displayed item wording and response scale against LimeSurvey."}):A(a,{code:"limesurvey-array-expanded",title:"LimeSurvey Array rows were converted to separate rating items",detail:"The source array heading is combined with each visible row label when both contain text. Review the converted wording and order."});for(const _ of x){const V=_.qid||_.title||`${b}-row`,be=F(D,"qid",_.qid,"question",c)||_.question,ne=F(D,"qid",_.qid,"help",c)||_.help,Ue=F(D,"qid",_.qid,"script",c)||_.script;if(I(Ue||"")||me.test(ne||"")||fe.test(ne||"")||he.test(ne||"")||Qe(ne||"")){a.unsupported.push({code:"limesurvey-array-row-content",title:`${_.title||V} contains unsupported row content`,detail:"Array-row scripts, dynamic content and help text are not imported."});continue}const ie=I(be??"")?U(be,"Array row text",_.title||V,a):null,_e=M&&ie?`${M}: ${ie}`:M||ie;if(!_e){a.unsupported.push({code:"missing-visible-text",title:"Array item text is empty",detail:`${_.title||V} and its parent question contain no participant-visible text.`});continue}const we={sourceId:`${s.title||b}/${_.title||V}`,name:x.length===1&&!ie?s.title||`Item ${L.length+1}`:_.title||s.title||`Item ${L.length+1}`,prompt:_e,values:[...H.values],labels:[...H.labels]};L.push(we),a.imported.push(X(we))}}else a.unsupported.push({code:"limesurvey-unsupported-question",title:`${s.title||b} uses unsupported LimeSurvey type ${s.type||"unknown"}`,detail:"Supported in this release: List (Radio), 5 Point Choice and reviewed Array rating rows."})}const ve=B?.description||T.description||E?.surveyls_description||E?.surveyls_welcometext||"",Me=I(ve)?U(ve,"Survey introduction","surveys_languagesettings",a):null,Fe=ke(k,`Imported from ${h}`,Me??"Answer each imported item about the task that you have just completed.",L,a);return ye(u,h,t,k,Fe,a,{groupOptions:le,selectedGroupId:T.gid})}function bt(e,t){const n=t.toLowerCase().split(".").at(-1),r=e.trimStart();if(n==="qsf"||r.startsWith("{")&&/"SurveyElements"/.test(e))return"qualtrics-qsf";if(n==="lss"||r.startsWith("<")&&/<LimeSurveyDocType>Survey</.test(e))return"limesurvey-lss";if(n==="lsg"||r.startsWith("<")&&/<LimeSurveyDocType>Group</.test(e))return"limesurvey-lsg";throw new Error("Choose a Qualtrics .qsf file or a LimeSurvey .lss or .lsg file.")}function xe(e,t,n="auto",r){const i=new TextEncoder().encode(e).length;if(!e.trim())throw new Error("The selected questionnaire export is empty.");if(i>lt)throw new Error("The selected questionnaire export is larger than the 2 MB review limit.");const o=bt(e,t);if(n!=="auto"&&n!==o){const u=o==="qualtrics-qsf"?"Qualtrics QSF":o==="limesurvey-lss"?"LimeSurvey LSS":"LimeSurvey LSG";throw new Error(`The selected file looks like ${u}, not the chosen format.`)}return o==="qualtrics-qsf"?mt(e,t):vt(e,t,r)}var _t=Object.defineProperty,wt=Object.getOwnPropertyDescriptor,y=(e,t,n,r)=>{for(var i=r>1?void 0:r?wt(t,n):t,o=e.length-1,u;o>=0;o--)(u=e[o])&&(i=(r?u(t,n,i):u(i))||i);return r&&i&&_t(t,n,i),i};const St=de.trim().split(/\r?\n/).filter(Boolean).length,W=pe.match(/var bridgeBuild = '([^']+)'/)?.[1]??"unidentified";function At(e){const t=Array.isArray(e)?e:[e];return t.length>0&&t.some(n=>{if(!n||typeof n!="object")return!1;const r=n;return"study"in r&&"responses"in r&&"result"in r})}function It(e){const t="PASTE_THE_GENERATED_PARTICIPANT_PAGE_URL_HERE";if(!e||e.includes(t))throw new Error("A generated participant URL is required for the Qualtrics question HTML.");const n=e.replace(/&/g,"&amp;").replace(/"/g,"&quot;");return rt.trim().replace(t,n)}function Tt(e){const t=e?["Questionnaire:","${e://Field/__js_AQP_INSTRUMENT_NAME}","","${e://Field/__js_AQP_SCORE_NAME}:","${e://Field/__js_AQP_PRIMARY_SCORE}"].join(`
`):"";return it.replace("{{OPTIONAL_SCORE_BLOCK}}",t).replace(/\n{3,}/g,`

`).trim()}let g=class extends He{constructor(){super(...arguments),this.instrumentId=Ve,this.customDefinition=null,this.customDraft=qe(),this.customBuilderOpen=!1,this.platformImportSource="auto",this.platformImportReview=null,this.platformImportConfirmed=!1,this.platformImportSelectedGroupId="",this.platformImportContents="",this.platformImportFileName="",this.studyId="",this.studyTitle="",this.taskLabel="",this.showScoreToParticipant=!1,this.showSimpleLanguage=!1,this.answerMode="standard",this.largeText=!1,this.audioGuidance=!1,this.recoveryEnabled=!0,this.participantAdjustmentPolicy="participant-choice",this.voiceInputAvailable=!0,this.gazeInputAvailable=!1,this.collectionMode="local",this.qualtricsSurveyUrl="",this.generatedConfig=null,this.participantUrl="",this.message="",this.definitionConfirmation="",this.configurationConfirmation="",this.errorMessage="",this.completedResults=[],this.selectInstrument=e=>{const t=e.currentTarget.value,n=this.availableDefinitions.find(r=>r.id===t)??null;n&&(this.instrumentId=t,n.supports.simplerExplanations||(this.showSimpleLanguage=!1),n.supports.smileyLandmarks||(this.answerMode="standard"),this.generatedConfig=null,this.participantUrl="",this.definitionConfirmation=`${n.name} ${n.version} selected. Complete the study details, then generate a new configuration.`,this.configurationConfirmation="",this.message=`${n.name} selected. Generate a new configuration before testing.`)},this.importPlatformQuestionnaire=async e=>{const t=e.currentTarget,n=t.files?.[0];if(n){this.errorMessage="",this.definitionConfirmation="",this.platformImportReview=null,this.platformImportConfirmed=!1;try{const r=await n.text();this.platformImportContents=r,this.platformImportFileName=n.name,this.platformImportSelectedGroupId="";const i=xe(r,n.name,this.platformImportSource);this.platformImportReview=i,i.draft&&(this.customDraft=structuredClone(i.draft)),this.message=i.requiresGroupSelection?"Choose one LimeSurvey questionnaire group to review.":i.canConvert?"Import review ready. Check every section and confirm scoring before conversion.":"Import review found unsupported content. No partial questionnaire was created.",this.revealConductorResult("#platform-import-review")}catch(r){this.showError(r instanceof Error?r.message:"The questionnaire export could not be reviewed.")}finally{t.value=""}}},this.reviewSelectedLimeSurveyGroup=()=>{if(!(!this.platformImportContents||!this.platformImportFileName||!this.platformImportSelectedGroupId)){this.errorMessage="",this.definitionConfirmation="",this.platformImportConfirmed=!1;try{const e=xe(this.platformImportContents,this.platformImportFileName,this.platformImportSource,this.platformImportSelectedGroupId);this.platformImportReview=e,e.draft&&(this.customDraft=structuredClone(e.draft)),this.message=e.canConvert?"Selected group review ready. Check every section and confirm scoring before conversion.":"The selected group contains unsupported content. No partial questionnaire was created.",this.revealConductorResult("#platform-import-review")}catch(e){this.showError(e instanceof Error?e.message:"The selected LimeSurvey group could not be reviewed.")}}},this.usePlatformImport=()=>{const e=this.platformImportReview;if(!(!e?.canConvert||!e.draft||!this.platformImportConfirmed)){this.errorMessage="",this.definitionConfirmation="";try{const t=$e(this.customDraft);this.activateCustomDefinition(t,"imported")}catch(t){this.showError(t instanceof Error?t.message:"The reviewed questionnaire could not be converted.")}}},this.addCustomItem=()=>{this.customDraft.items.length>=j||(this.customDraft={...this.customDraft,items:[...this.customDraft.items,se({name:`Item ${this.customDraft.items.length+1}`})]})},this.useCustomDraft=()=>{this.errorMessage="",this.definitionConfirmation="";try{this.activateCustomDefinition($e(this.customDraft),"validated")}catch(e){this.showError(e instanceof Error?e.message:"The custom questionnaire could not be validated.")}},this.importCustomDefinition=async e=>{const t=e.currentTarget,n=t.files?.[0];if(n){this.errorMessage="",this.definitionConfirmation="";try{const r=Ne(JSON.parse(await n.text()));this.activateCustomDefinition(r,"imported")}catch(r){this.showError(r instanceof Error?r.message:"The questionnaire definition file could not be read.")}finally{t.value=""}}},this.downloadCustomDefinition=()=>{this.customDefinition&&re(at(this.customDefinition),JSON.stringify(this.customDefinition,null,2),"application/json")},this.resetCustomDraft=()=>{this.customDraft=qe(),this.platformImportReview=null,this.platformImportConfirmed=!1,this.platformImportSelectedGroupId="",this.platformImportContents="",this.platformImportFileName="",this.message="Custom questionnaire builder fields reset. The selected questionnaire is unchanged until you validate a new definition."},this.generateParticipantLink=()=>{this.errorMessage="",this.configurationConfirmation="";try{const e=We({instrumentId:this.instrumentId,...this.customDefinition?.id===this.instrumentId?{questionnaireDefinition:this.customDefinition}:{},studyId:this.studyId,studyTitle:this.studyTitle,taskLabel:this.taskLabel,showScoreToParticipant:this.showScoreToParticipant,support:this.currentSupportConfig(),collection:this.currentCollectionConfig()});this.useConfiguration(e),this.configurationConfirmation="Participant link and configuration generated. The ready-to-use files and link are shown below.",this.message="",this.revealConductorResult("#configuration-ready-panel")}catch(e){this.showError(e instanceof Error?e.message:"The study configuration could not be generated.")}},this.copyParticipantLink=async()=>{this.participantUrl&&await this.copySetupAsset(this.participantUrl,"participant link")},this.copySetupAsset=async(e,t)=>{try{if(!navigator.clipboard?.writeText)throw new Error("Clipboard API unavailable.");await navigator.clipboard.writeText(e),this.message=`${t.charAt(0).toUpperCase()}${t.slice(1)} copied.`}catch{this.message=`Automatic copy was unavailable. Select and copy the ${t} from its text box.`}},this.downloadConfiguration=()=>{this.generatedConfig&&re(`${this.generatedConfig.studyId}-${this.generatedConfig.configId}.json`,JSON.stringify(this.generatedConfig,null,2),"application/json")},this.importConfiguration=async e=>{const t=e.currentTarget,n=t.files?.[0];if(n){this.errorMessage="",this.configurationConfirmation="";try{const r=JSON.parse(await n.text()),i=ze(r);if(!i)throw At(r)?new Error("This is a completed result file, not a study configuration. Import the JSON downloaded from Configuration ready."):new Error("This is not a valid Version 0.8 study configuration or supported Version 0.7 configuration.");this.useConfiguration(i),this.configurationConfirmation="Configuration imported and participant link reproduced. The configuration ID and files are shown below.",this.message="",this.revealConductorResult("#configuration-ready-panel")}catch(r){this.showError(r instanceof Error?r.message:"The configuration file could not be read.")}finally{t.value=""}}},this.refreshResults=()=>{this.completedResults=Ye()},this.exportResultsJson=()=>{this.completedResults.length&&re(`accessible-questionnaire-results-${new Date().toISOString().slice(0,10)}.json`,JSON.stringify(this.completedResults,null,2),"application/json")},this.exportResultsCsv=()=>{this.completedResults.length&&re(`accessible-questionnaire-results-${new Date().toISOString().slice(0,10)}.csv`,`\uFEFF${Xe(this.completedResults)}`,"text/csv")},this.eraseResults=()=>{window.confirm("Erase every completed questionnaire record stored by this site in this browser? Confirm only after checking the exported files.")&&(Ze(),this.refreshResults(),this.message="Local completed records erased.")}}connectedCallback(){super.connectedCallback(),this.refreshResults(),window.addEventListener("storage",this.refreshResults)}disconnectedCallback(){window.removeEventListener("storage",this.refreshResults),super.disconnectedCallback()}createRenderRoot(){return this}get definition(){return Ke(this.instrumentId,this.customDefinition??void 0)}get availableDefinitions(){return this.customDefinition?[...Ae,this.customDefinition]:Ae}render(){return m`
      <a class="skip-link" href="#conductor-main">Skip to study setup</a>
      <main class="app-shell conductor-shell" id="conductor-main">
        <header class="app-header">
          <p class="eyebrow">
            Study conductor · Version ${ue} · Qualtrics package ${W}
          </p>
        ...
        <aside class="boundary-note important-boundary" aria-labelledby="current-generator-heading">
          <h2 id="current-generator-heading">Current Qualtrics generator: ${W}</h2>
          <p>
            Every generated JavaScript block must contain
            <code>var bridgeBuild = '${W}';</code>. If it shows another value, that browser tab is
            running a stale conductor build. Close that tab and reopen the versioned
            <a href="study.html?package=${W}">Prepare a study page</a> before copying anything.
          </p>
        </aside>
          ...
          <div class="form-grid">
            <label class="full-width">
              ...
              <select @change=${this.selectInstrument}>
                ${this.availableDefinitions.map(e=>m`<option
                  ...
                  >
                    ${e.name} · ${e.version}${Re(e.id)?"":" · researcher supplied"}
                  </option>`)}
              </select>
            </label>
            <aside
            ...
            >
              ...
              <span>
                ...
                ${ce(this.definition).length}
                ${this.definition.scale.type.replace("-"," ")} response values,
                ${et(this.definition).length} comparisons,
        ...
        <section class="panel conductor-panel" aria-labelledby="support-config-heading">
          ...
          ${this.definition.supports.smileyLandmarks?m`<fieldset class="answer-mode-control conductor-answer-mode">
                ...
                <label>
                  ...
                  <span>
                    <strong>Standard ${ce(this.definition).length}-value scale</strong>
                ...
                ${this.definition.shortName} uses its standard ${ce(this.definition).length}-value
        ...
        <section class="panel conductor-panel" aria-labelledby="remote-heading">
          ...
          <p>
            Version ${ue} includes a Qualtrics parent bridge. The participant page sends a complete record only to the
      ...
      >
        ...
        >
          ...
          <p>
            ...
            1–${j} required single-choice items that
        ...
        <fieldset class="custom-items">
          ...
          <button
            class="secondary-button"
            type="button"
            ?disabled=${this.customDraft.items.length>=j}
        ...
        <div class="form-grid">
          <label>
            ...
            <select
              ...
              @change=${t=>{this.platformImportSource=t.currentTarget.value,this.platformImportReview=null,this.platformImportConfirmed=!1,this.platformImportSelectedGroupId="",this.platformImportContents="",this.platformImportFileName=""}}
            >
    ...
    `}renderPlatformQuestionnaireReview(e){const t=(n,r,i,o)=>m`
        ...
        ${e.canConvert&&e.draft?m`
              <fieldset class="platform-import-confirmation">
                ...
                <fieldset class="platform-import-reverse-items">
                  ...
                  ${this.customDraft.items.map((n,r)=>m`
                    <label>
                      <input
                        data-platform-import-reverse=${r}
                        type="checkbox"
                        .checked=${n.reverseScored}
                        @change=${i=>this.updateCustomItem(r,"reverseScored",i.currentTarget.checked)}
                      />
                      <span>${r+1}. ${n.name}: ${n.prompt}</span>
                    </label>
                  `)}
                </fieldset>
    ...
    `}updateCustomDraft(e,t){this.customDraft={...this.customDraft,[e]:t},this.platformImportReview&&(this.platformImportConfirmed=!1)}updateCustomItem(e,t,n){this.customDraft={...this.customDraft,items:this.customDraft.items.map((r,i)=>i===e?{...r,[t]:n}:r)},this.platformImportReview&&(this.platformImportConfirmed=!1)}removeCustomItem(e){this.customDraft.items.length!==1&&(this.customDraft={...this.customDraft,items:this.customDraft.items.filter((t,n)=>n!==e)})}activateCustomDefinition(e,t){this.customDefinition=e,this.instrumentId=e.id,e.supports.simplerExplanations||(this.showSimpleLanguage=!1),this.answerMode="standard",this.generatedConfig=null,this.participantUrl="";const n=t==="imported"?"imported, validated and selected":"validated and selected";this.definitionConfirmation=`${e.name} ${e.version} ${n}. Complete the study details, then generate a new configuration.`,this.configurationConfirmation="",this.message="",this.revealConductorResult("#selected-questionnaire-summary")}booleanOption(e,t,n,r=""){return m`<label class="toggle-card conductor-toggle">
      <input type="checkbox" .checked=${t} @change=${i=>n(i.currentTarget.checked)} />
      <span><strong>${e}</strong>${r?m`<small>${r}</small>`:Q}</span>
    </label>`}currentSupportConfig(){return{showSimpleLanguage:this.definition.supports.simplerExplanations&&this.showSimpleLanguage,answerMode:this.definition.supports.smileyLandmarks?this.answerMode:"standard",largeText:this.largeText,audioGuidance:this.audioGuidance,recoveryEnabled:this.recoveryEnabled,participantAdjustmentPolicy:this.participantAdjustmentPolicy,voiceInputAvailable:this.voiceInputAvailable,gazeInputAvailable:this.gazeInputAvailable}}currentCollectionConfig(){if(this.collectionMode==="local")return{mode:"local"};const e=tt(this.qualtricsSurveyUrl);if(!e)throw new Error("Enter a valid HTTPS Qualtrics survey or preview URL for central collection.");if(e===window.location.origin)throw new Error("The Qualtrics origin must be different from this GitHub Pages website.");return{mode:"qualtrics",parentOrigin:e}}useConfiguration(e){e.questionnaireDefinition&&(this.customDefinition=e.questionnaireDefinition),this.generatedConfig=e,this.instrumentId=e.instrumentId,this.studyId=e.studyId,this.studyTitle=e.studyTitle,this.taskLabel=e.taskLabel,this.showScoreToParticipant=e.showScoreToParticipant,this.showSimpleLanguage=e.support.showSimpleLanguage,this.answerMode=e.support.answerMode,this.largeText=e.support.largeText,this.audioGuidance=e.support.audioGuidance,this.recoveryEnabled=e.support.recoveryEnabled,this.participantAdjustmentPolicy=e.support.participantAdjustmentPolicy,this.voiceInputAvailable=e.support.voiceInputAvailable,this.gazeInputAvailable=e.support.gazeInputAvailable,this.collectionMode=e.collection.mode,this.qualtricsSurveyUrl=e.collection.mode==="qualtrics"?e.collection.parentOrigin:"",this.participantUrl=nt(new URL("index.html",window.location.href).toString(),e)}qualtricsIframeHtml(){return!this.generatedConfig||this.generatedConfig.collection.mode!=="qualtrics"?"":It(this.participantUrl)}renderQualtricsSetup(){const e=this.qualtricsIframeHtml(),t=Tt(this.generatedConfig?.showScoreToParticipant===!0);return m`
      <div class="qualtrics-setup" role="region" aria-labelledby="qualtrics-setup-heading">
        ...
        <p>
          ...
          platform ${ue}; Qualtrics bridge ${W}.
        ...
        <aside class="boundary-note important-boundary">
          <p>
            ...
            ${W} and say that diagnostic fields were staged. Then complete one
        ...
        <ol class="qualtrics-install-steps">
          ...
          <li>
            ...
            <label for="qualtrics-embedded-fields">
              <strong>${St} Embedded Data field names</strong>
            </label>
            <textarea
            ...
            <button
              class="secondary-button"
              type="button"
              @click=${()=>this.copySetupAsset(de.trim(),"Embedded Data field list")}
            >
          ...
          <li>
            ...
            <button
              class="secondary-button"
              type="button"
              @click=${()=>this.copySetupAsset(pe.trim(),"question JavaScript")}
            >
    ...
    `}showError(e){this.errorMessage=e,this.updateComplete.then(()=>{const t=this.querySelector("#conductor-error");t&&Ie(t)})}revealConductorResult(e){this.updateComplete.then(()=>{const t=this.querySelector(e);t&&Ie(t,{block:"start"})})}};y([v()],g.prototype,"instrumentId",2);y([v()],g.prototype,"customDefinition",2);y([v()],g.prototype,"customDraft",2);y([v()],g.prototype,"customBuilderOpen",2);y([v()],g.prototype,"platformImportSource",2);y([v()],g.prototype,"platformImportReview",2);y([v()],g.prototype,"platformImportConfirmed",2);y([v()],g.prototype,"platformImportSelectedGroupId",2);y([v()],g.prototype,"studyId",2);y([v()],g.prototype,"studyTitle",2);y([v()],g.prototype,"taskLabel",2);y([v()],g.prototype,"showScoreToParticipant",2);y([v()],g.prototype,"showSimpleLanguage",2);y([v()],g.prototype,"answerMode",2);y([v()],g.prototype,"largeText",2);y([v()],g.prototype,"audioGuidance",2);y([v()],g.prototype,"recoveryEnabled",2);y([v()],g.prototype,"participantAdjustmentPolicy",2);y([v()],g.prototype,"voiceInputAvailable",2);y([v()],g.prototype,"gazeInputAvailable",2);y([v()],g.prototype,"collectionMode",2);y([v()],g.prototype,"qualtricsSurveyUrl",2);y([v()],g.prototype,"generatedConfig",2);y([v()],g.prototype,"participantUrl",2);y([v()],g.prototype,"message",2);y([v()],g.prototype,"definitionConfirmation",2);y([v()],g.prototype,"configurationConfirmation",2);y([v()],g.prototype,"errorMessage",2);y([v()],g.prototype,"completedResults",2);g=y([Je("study-conductor-app")],g);
