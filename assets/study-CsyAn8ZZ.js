import{w as Ce,g as ge,x as $e,M as le,a as m,t as Ee,i as Qe,D as Pe,m as W,y as Re,z as xe,u as Ne,o as De,B as ke,h as Le,C as ue,P as ee,A as T,l,k as te,j as Oe,E as Me,F as Fe,f as ce}from"./shared-d6lA0Eal.js";const ne=`__js_AQP_ACCEPTED
...
`,L=20;let de=0;function Y(e={}){return de+=1,{key:`custom-item-${de}`,name:"",prompt:"",lowAnchor:"",highAnchor:"",simpleExplanation:"",reverseScored:!1,...e}}function pe(){return{name:"",shortName:"",version:"1.0.0",description:"A researcher-supplied questionnaire.",introPrompt:"Answer each item about the task that you have just completed.",sourceLabel:"Researcher-supplied questionnaire",sourceUrl:"",scaleType:"agreement",minimum:1,maximum:5,step:1,scoreName:"Questionnaire score",aggregation:"mean",items:[Y({name:"Item 1"}),Y({name:"Item 2"})]}}function Q(e,t){const n=e.trim().replace(/\s+/g," ");if(!n)throw new Error(`${t} is required.`);return n}function Be(e){return e.trim().toLowerCase().normalize("NFKD").replace(/[\u0300-\u036f]/g,"").replace(/[^a-z0-9]+/g,"-").replace(/^-+|-+$/g,"").slice(0,50)}function Je(e,t){const n=t.trim()||e.trim();return n.length<=240?n:`${n.slice(0,237).trimEnd()}...`}function me(e){if(!Number.isInteger(e.minimum)||!Number.isInteger(e.maximum)||!Number.isInteger(e.step))throw new Error("Scale minimum, maximum and step must be whole numbers.");if(!Array.isArray(e.items)||e.items.length<1||e.items.length>L)throw new Error(`Add between 1 and ${L} questionnaire items.`);const t=Q(e.name,"Questionnaire name"),n=Q(e.shortName,"Short name"),i=Be(n||t);if(!i)throw new Error("Questionnaire name must contain at least one Latin letter or number for its stable ID.");const s=e.items.map((a,v)=>{const w=Q(a.prompt,`Item ${v+1} question`),c=a.simpleExplanation.trim().replace(/\s+/g," ");return{id:`item-${String(v+1).padStart(2,"0")}`,name:Q(a.name,`Item ${v+1} label`),prompt:w,...c?{simpleExplanation:c}:{},shortMeaning:Je(w,c),lowAnchor:Q(a.lowAnchor,`Item ${v+1} low endpoint`),highAnchor:Q(a.highAnchor,`Item ${v+1} high endpoint`),...a.responseLabels?{responseLabels:a.responseLabels}:{}}}),r=e.items.flatMap((a,v)=>a.reverseScored?[`item-${String(v+1).padStart(2,"0")}`]:[]),g=s.every(a=>a.simpleExplanation),y=e.aggregation==="mean"?e.minimum:e.minimum*s.length,h=e.aggregation==="mean"?e.maximum:e.maximum*s.length,A={schemaVersion:1,id:`custom-${i}`,version:Q(e.version,"Questionnaire version"),name:t,shortName:n,description:Q(e.description,"Questionnaire description"),introPrompt:Q(e.introPrompt,"Participant instruction"),officialContentNotice:"This questionnaire definition was supplied by the study conductor. Its wording, use and interpretation must match the approved study protocol.",source:{label:Q(e.sourceLabel,"Source or authorship label"),...e.sourceUrl.trim()?{url:e.sourceUrl.trim()}:{}},scale:{type:e.scaleType,minimum:e.minimum,maximum:e.maximum,step:e.step},items:s,scoring:{strategy:e.aggregation==="mean"?"mean-v1":"sum-v1",scoreName:Q(e.scoreName,"Score name"),minimum:y,maximum:h,...r.length?{reverseItemIds:r}:{}},supports:{simplerExplanations:g,smileyLandmarks:!1}};return ye(A)}function ye(e){const t=Ce(e);if(ge(t.id))throw new Error("A custom questionnaire cannot replace a built-in questionnaire ID.");if(!t.id.startsWith("custom-"))throw new Error("A custom questionnaire ID must start with custom-.");if(t.scoring.strategy!=="mean-v1"&&t.scoring.strategy!=="sum-v1")throw new Error("A custom questionnaire must use the reviewed mean or sum scorer.");if(t.pairwise)throw new Error("Custom questionnaires do not support pairwise comparisons.");if(t.landmarks||t.supports.smileyLandmarks)throw new Error("Custom questionnaires do not support smiley landmarks.");const n=$e(t);if(n>le)throw new Error(`The questionnaire definition is ${n} bytes; the participant-link limit is ${le} bytes.`);return t}function Ge(e){return`${e.id}-${e.version.replace(/[^A-Za-z0-9._-]+/g,"-")}.questionnaire.json`}const He=2e6,ve=/<\s*(?:script|iframe|object|embed|style)\b|(?:href|src)\s*=\s*["']?\s*javascript:|\son[a-z]+\s*=/i,be=/<\s*(?:img|picture|video|audio|canvas|svg|math|form|input|button|select|textarea|table)\b/i,_e=/\$\{(?:e|q|lm|gr)?:?\/?\/|q:\/\/|\{(?:if|TOKEN|INSERTANS|[A-Za-z][A-Za-z0-9_.]*\.(?:NAOK|shown))\b/i;function I(e){return e&&typeof e=="object"&&!Array.isArray(e)?e:null}function S(e){return typeof e=="string"||typeof e=="number"?String(e):""}function x(e){return e.replace(/\s+/g," ").trim()}function $(e,t){e.confirmationCodes.has(t.code)||(e.confirmationCodes.add(t.code),e.confirmations.push(t))}function F(e,t,n,i){const s=S(e);if(!s.trim())return i.unsupported.push({code:"missing-visible-text",title:`${t} is empty`,detail:`${n} does not contain participant-visible text.`}),null;if(ve.test(s)||_e.test(s))return i.unsupported.push({code:"unsafe-dynamic-content",title:`${t} contains executable or dynamic content`,detail:`${n} contains script, an event handler, a JavaScript URL or survey-expression text. The importer does not execute or approximate it.`}),null;if(be.test(s))return i.unsupported.push({code:"unsupported-structured-content",title:`${t} contains media, a table or an interactive control`,detail:`${n} cannot be represented as one safe plain-text rating item without changing participant-visible content.`}),null;const g=new DOMParser().parseFromString(`<body>${s}</body>`,"text/html"),y=x(g.body.textContent??"");return y?((/<[^>]+>/.test(s)||y!==x(s))&&$(i,{code:"plain-text-normalisation",title:"Formatting was converted to plain text",detail:"Imported wording is rendered as safe plain text. Review it against the source export before use."}),y):(i.unsupported.push({code:"missing-visible-text",title:`${t} has no readable text`,detail:`${n} contains no participant-visible text after safe text extraction.`}),null)}function X(e){const t=typeof e=="number"?e:Number(S(e).trim());return Number.isInteger(t)?t:null}function se(e){if(e.length<2||e.some(n=>n<0||n>100)||new Set(e).size!==e.length)return!1;const t=e[1]-e[0];return t>0&&e.every((n,i)=>i===0||n-e[i-1]===t)}function we(e,t){if(!Array.isArray(e)||e.length!==Object.keys(t).length)return null;const n=e.map(String);return new Set(n).size===n.length&&n.every(i=>i in t)?n:null}function Se(e,t,n,i){const s=I(e);return F(s?.Display,t,n,i)}function Ve(e){return e.flatMap(n=>n.labels).map(n=>n.toLowerCase()).some(n=>n.includes("agree")||n.includes("disagree"))?"agreement":"semantic-differential"}function We(e){const t=e.split(/\s+/).map(i=>i.replace(/[^A-Za-z0-9]/g,"")[0]??"").join("").slice(0,12);return t?t.toUpperCase():e.replace(/[^A-Za-z0-9]+/g,"").slice(0,12)||"IMPORTED"}function Ae(e,t,n,i,s){if(!i.length)return s.unsupported.push({code:"no-supported-items",title:"No supported questionnaire items were found",detail:"This release needs at least one required, ordered single-choice rating item."}),null;if(i.length>L)return s.unsupported.push({code:"too-many-items",title:"The questionnaire has too many items for a participant link",detail:`${i.length} supported items were found; this release accepts at most ${L}.`}),null;const r=i[0].values;if(!se(r))return s.unsupported.push({code:"unsupported-response-values",title:"Response values are not one increasing whole-number scale",detail:"Values must be unique whole numbers from 0 to 100 with one constant positive step."}),null;const g=r.join("|");return i.some(y=>y.values.join("|")!==g)?(s.unsupported.push({code:"mixed-response-scales",title:"Items use different response values",detail:"The current participant interface requires every imported item to share the same ordered numeric scale."}),null):($(s,{code:"review-scoring",title:"Scoring and reverse scoring require confirmation",detail:"Answer recodes do not establish a questionnaire score. Choose reviewed mean or sum, then mark any reverse-scored items."}),$(s,{code:"review-scale-type",title:"The scale description requires confirmation",detail:"Confirm whether the imported scale should be described as agreement or semantic differential."}),{name:e.slice(0,120),shortName:We(e),version:"1.0.0",description:`Questionnaire imported from ${t}.`,introPrompt:n.slice(0,400),sourceLabel:t,sourceUrl:"",scaleType:Ve(i),minimum:r[0],maximum:r.at(-1),step:r[1]-r[0],scoreName:"Questionnaire score",aggregation:"mean",items:i.map(y=>Y({name:y.name.slice(0,120),prompt:y.prompt.slice(0,1e3),lowAnchor:y.labels[0].slice(0,80),highAnchor:y.labels.at(-1).slice(0,80),responseLabels:Object.fromEntries(y.values.map((h,A)=>[String(h),y.labels[A].slice(0,120)]))}))})}function re(e){return{code:`item-${e.sourceId}`,title:`${e.name}: imported`,detail:`${e.sourceId}; “${e.prompt}”; ordered choices: `+e.values.map((t,n)=>`${t} = ${e.labels[n]}`).join("; ")}}function Ie(e,t,n,i,s,r){return{source:e,sourceName:t,fileName:n,title:i,draft:s,imported:r.imported,confirmations:r.confirmations,unsupported:r.unsupported,canConvert:!!s&&r.unsupported.length===0}}function Te(){return{imported:[],confirmations:[],unsupported:[],confirmationCodes:new Set}}function he(e,t){const n=I(e);if(!n)throw new Error(t);return n}function ze(e,t){const n=e.find(c=>c.Element==="BL"),i=Array.isArray(n?.Payload)?n.Payload.map(I).filter(c=>!!c):[],s=i.filter(c=>c.Type!=="Trash"),r=new Set(i.filter(c=>c.Type==="Trash").flatMap(c=>Array.isArray(c.BlockElements)?c.BlockElements:[]).map(I).filter(c=>!!c).map(c=>S(c.QuestionID)).filter(Boolean));r.size&&$(t,{code:"qualtrics-trash-ignored",title:"Questions in the Qualtrics Trash block were ignored",detail:`${r.size} deleted question${r.size===1?"":"s"} will not appear in the converted questionnaire.`});const g=e.find(c=>c.Element==="FL"),y=I(g?.Payload),h=Array.isArray(y?.Flow)?y.Flow.map(I).filter(c=>!!c):[];if(s.length!==1||h.length!==1||h[0].Type!=="Block")return t.unsupported.push({code:"qualtrics-complex-flow",title:"Qualtrics flow is outside the supported single-block subset",detail:"Use one ordinary question block only. Branches, randomisers, embedded-data flow and multiple blocks are not removed or flattened."}),{order:[],trashQuestionIds:r};const A=S(h[0].ID),a=s.find(c=>S(c.ID)===A);if(!a)return t.unsupported.push({code:"qualtrics-missing-block",title:"The active Qualtrics block could not be resolved",detail:`Survey Flow references ${A||"an unknown block"}.`}),{order:[],trashQuestionIds:r};const v=Array.isArray(a.BlockElements)?a.BlockElements:[],w=[];for(const c of v){const q=I(c);if(!q||q.Type!=="Question"||!S(q.QuestionID)){t.unsupported.push({code:"qualtrics-non-question-block-item",title:"The Qualtrics block contains unsupported content",detail:"Only ordered question entries are supported in the imported block."});continue}(q.SkipLogic||q.DisplayLogic)&&t.unsupported.push({code:"qualtrics-question-logic",title:`${S(q.QuestionID)} uses question logic`,detail:"Skip and display logic are not imported."}),w.push(S(q.QuestionID))}return{order:w,trashQuestionIds:r}}function Ye(e){return["QuestionJS","JavaScript","DisplayLogic","SkipLogic","ChoiceDisplayLogic","CarryForward","Randomization","ChoiceRandomization"].some(t=>e[t]!==void 0&&e[t]!==null)}function fe(e,t,n,i,s){const r=I(e[t]);if(!r)return s.unsupported.push({code:"qualtrics-missing-scale",title:`${i} has no answer scale`,detail:`${t} must contain every visible answer option.`}),null;const g=we(e[n],r);if(!g)return s.unsupported.push({code:"qualtrics-unknown-answer-order",title:`${i} has no reliable answer order`,detail:`${n} must list every ${t.toLowerCase()} entry exactly once.`}),null;const y=I(e.RecodeValues);let h;if(y){const a=g.map(v=>X(y[v]));if(a.some(v=>v===null))return s.unsupported.push({code:"qualtrics-nonnumeric-recodes",title:`${i} has missing or non-integer recode values`,detail:"Every visible option needs an explicit whole-number recode."}),null;h=a}else{if(!g.every((v,w)=>v===String(w+1)))return s.unsupported.push({code:"qualtrics-missing-scale",title:`${i} has no explicit recode table or default sequential choice IDs`,detail:"Without RecodeValues, ChoiceOrder must explicitly list the default IDs 1 through N. The importer will not infer values from other object keys."}),null;h=g.map((v,w)=>w+1),$(s,{code:"qualtrics-default-recodes",title:"Qualtrics default sequential recodes were used",detail:"The QSF omits RecodeValues and explicitly orders default choice IDs 1 through N. Confirm these values against the source survey before conversion."})}const A=g.map((a,v)=>Se(r[a],`Answer ${v+1}`,`${i}/${a}`,s));return A.some(a=>a===null)?null:{values:h,labels:A}}function Xe(e,t){let n;try{n=JSON.parse(e)}catch{throw new Error("The Qualtrics QSF file is not valid JSON.")}const i=he(n,"The Qualtrics QSF root must be a JSON object."),s=he(i.SurveyEntry,"The file does not contain a Qualtrics SurveyEntry.");if(!Array.isArray(i.SurveyElements))throw new Error("The file does not contain Qualtrics SurveyElements.");const r=Te(),g=x(S(s.SurveyName))||"Imported Qualtrics questionnaire",y=i.SurveyElements.map(I).filter(d=>!!d),{order:h,trashQuestionIds:A}=ze(y,r),a=new Set(y.map(d=>S(d.Element)).filter(d=>d&&!["BL","FL","SQ"].includes(d)));a.size&&$(r,{code:"qualtrics-platform-settings-not-imported",title:"Qualtrics platform and presentation settings are not imported",detail:`The following non-question element types remain in Qualtrics only: ${[...a].sort().join(", ")}. Review the converted participant presentation.`});const v=new Map(y.filter(d=>d.Element==="SQ").map(d=>[S(d.PrimaryAttribute),I(d.Payload)]));if(h.length){const d=new Set(h);for(const _ of v.keys())!_||d.has(_)||A.has(_)||r.unsupported.push({code:"qualtrics-unreferenced-question",title:`${_} is outside the imported active block`,detail:"The importer will not silently omit an active question that is not represented by the supported single-block flow."})}const w=[];for(const d of h){const _=v.get(d);if(!_){r.unsupported.push({code:"qualtrics-missing-question",title:`${d} is missing`,detail:"The active block references a question that is not present in SurveyElements."});continue}if(Ye(_)){r.unsupported.push({code:"qualtrics-unsupported-behaviour",title:`${d} contains logic, randomisation or code`,detail:"The importer does not execute or remove question behaviour."});continue}const G=I(_.Validation);if(I(G?.Settings)?.ForceResponse!=="ON"){r.unsupported.push({code:"qualtrics-optional-question",title:`${d} is not a forced-response question`,detail:"The participant platform currently requires every imported item."});continue}const D=F(_.QuestionText,"Question text",d,r);if(!D)continue;const O=S(_.QuestionType),U=S(_.Selector),H=S(_.SubSelector),o=x(S(_.DataExportTag))||d;if(O==="MC"&&U==="SAVR"){const f=fe(_,"Choices","ChoiceOrder",d,r);if(!f)continue;const E={sourceId:d,name:o,prompt:D,...f};w.push(E),r.imported.push(re(E));continue}if(O==="Matrix"&&U==="Likert"&&H==="SingleAnswer"){const f=fe(_,"Answers","AnswerOrder",d,r),E=I(_.Choices),C=E?we(_.ChoiceOrder,E):null;if(!f||!E||!C){C||r.unsupported.push({code:"qualtrics-unknown-row-order",title:`${d} has no reliable matrix row order`,detail:"ChoiceOrder must list every matrix row exactly once."});continue}$(r,{code:"qualtrics-matrix-expanded",title:"Single-answer matrix rows were expanded into items",detail:"Review each generated item against the original matrix before conversion."}),C.forEach((B,V)=>{const R=Se(E[B],`Matrix row ${V+1}`,`${d}/${B}`,r);if(!R)return;const N={sourceId:`${d}/${B}`,name:R,prompt:`${D} — ${R}`,...f};w.push(N),r.imported.push(re(N))});continue}r.unsupported.push({code:"qualtrics-unsupported-question",title:`${d} uses an unsupported Qualtrics question type`,detail:`${O||"Unknown"} / ${U||"Unknown"} / ${H||"none"} is not converted. Supported: MC/SAVR and Matrix/Likert/SingleAnswer.`})}const q=(S(s.SurveyDescription).trim()?F(s.SurveyDescription,"Survey description","SurveyEntry",r):null)??"Answer each imported item about the task that you have just completed.",j=Ae(g,"Imported from Qualtrics QSF",q,w,r);return Ie("qualtrics-qsf","Qualtrics QSF",t,g,j,r)}function P(e,t){const n=[...e.documentElement.children].find(s=>s.localName===t);if(!n)return[];const i=[...n.children].find(s=>s.localName==="rows");return i?[...i.children].filter(s=>s.localName==="row").map(s=>Object.fromEntries([...s.children].map(r=>[r.localName,r.textContent??""]))):[]}function z(e,t,n,i,s){return e.find(r=>r[t]===n&&(!r.language||r.language===s))?.[i]??""}const Ze={answer_order:["normal"],array_filter_style:["0"],hidden:["0"],hide_tip:["0"],other_comment_mandatory:["0"],other_numbers_only:["0"],other_position:["default"],page_break:["0"],public_statistics:["0"],scale_export:["0"],statistics_graphtype:["0"],statistics_showgraph:["1"],time_limit_action:["1"],time_limit_disable_next:["0"],time_limit_disable_prev:["0"]};function Ke(e,t){if(!e.length)return;const n=e.filter(i=>{const s=i.attribute?.trim()??"",r=i.value?.trim()??"";return r?!Ze[s]?.includes(r):!1});if(n.length){const i=n.slice(0,6).map(s=>`${s.qid||"unknown question"}: ${s.attribute||"unknown"}=${s.value||""}`).join("; ");t.unsupported.push({code:"limesurvey-question-attributes",title:"The LimeSurvey export contains active or unknown question attributes",detail:`${i}${n.length>6?`; and ${n.length-6} more`:""}. These settings may change validation, randomisation, timing or presentation and are not discarded.`});return}$(t,{code:"limesurvey-default-question-attributes",title:"Default LimeSurvey question settings were not imported",detail:"Only empty or known default attributes were present; no active validation, randomisation or timing rule was found. Review the converted participant presentation."})}function et(e){if(!e.trim())return"";const n=new DOMParser().parseFromString(`<body>${e}</body>`,"text/html");return x(n.body.textContent??"")}function tt(e){const t=e.map(s=>X(s.sortorder));if(t.some(s=>s===null))return!1;const n=t;if(new Set(n).size!==n.length)return!1;const i=n[0];return(i===0||i===1)&&n.every((s,r)=>s===i+r)}function nt(e){return e.every((t,n)=>t.code===`A${String(n+1).padStart(3,"0")}`)}function it(e,t){if(/<!DOCTYPE|<!ENTITY|<\?xml-stylesheet/i.test(e))throw new Error("The LimeSurvey file contains a DTD, entity or stylesheet declaration and was not parsed.");const i=new DOMParser().parseFromString(e,"application/xml");if(i.querySelector("parsererror"))throw new Error("The LimeSurvey LSS file is not valid XML.");if(i.querySelector("LimeSurveyDocType")?.textContent?.trim()!=="Survey")throw new Error("The XML file is not a LimeSurvey survey-structure export.");const s=Te(),r=[...i.querySelectorAll("languages > language")].map(o=>o.textContent?.trim()??"").filter(Boolean),g=P(i,"surveys"),h=x(g[0]?.language??"")||r[0]||"";!h||r.length&&!r.includes(h)?s.unsupported.push({code:"limesurvey-base-language",title:"The LimeSurvey base language could not be resolved",detail:"The survey language must match one language declared in the LSS export."}):r.length>1&&$(s,{code:"limesurvey-base-language-only",title:`Only the LimeSurvey base language (${h}) will be imported`,detail:`Additional languages (${r.filter(o=>o!==h).join(", ")}) remain in LimeSurvey and are not included in the converted definition. Confirm the intended participant language.`});const A=P(i,"surveys_languagesettings"),a=A.find(o=>!o.surveyls_language||o.surveyls_language===h)??A[0],v=x(a?.surveyls_title??"")||"Imported LimeSurvey questionnaire",w=P(i,"groups"),c=P(i,"group_l10ns");w.length!==1&&s.unsupported.push({code:"limesurvey-multiple-groups",title:"The LimeSurvey export is outside the supported single-group subset",detail:"Multiple group headings and group-level navigation are not flattened."});for(const o of w){o.grelevance&&o.grelevance!=="1"&&s.unsupported.push({code:"limesurvey-group-relevance",title:`Group ${o.gid||""} uses relevance logic`,detail:"Group conditions are not imported."});const E=c.find(C=>C.gid===o.gid&&(!C.language||C.language===h))?.description??o.description??"";x(E)&&s.unsupported.push({code:"limesurvey-group-description",title:`Group ${o.gid||""} has participant-visible description text`,detail:"Group descriptions are not silently removed by this release."})}P(i,"conditions").length&&s.unsupported.push({code:"limesurvey-conditions",title:"The LimeSurvey export contains conditions",detail:"Branching and conditional relevance are not imported."});for(const[o,f]of[["assessments","assessment rules"],["defaultvalues","default answers"],["quotas","quota rules"],["quota_members","quota membership rules"],["quota_languages","localised quota messages"]])P(i,o).length&&s.unsupported.push({code:`limesurvey-${o.replace(/_/g,"-")}`,title:`The LimeSurvey export contains ${f}`,detail:`${f[0].toUpperCase()}${f.slice(1)} are not executed or silently discarded.`});Ke(P(i,"question_attributes"),s);const q=P(i,"questions"),j=P(i,"question_l10ns"),d=P(i,"answers"),_=P(i,"answer_l10ns"),G=new Map(w.map(o=>[o.gid,Number(o.group_order)||0])),oe=q.filter(o=>!o.parent_qid||o.parent_qid==="0").sort((o,f)=>(G.get(o.gid)??0)-(G.get(f.gid)??0)||(Number(o.question_order)||0)-(Number(f.question_order)||0)),D=[];q.some(o=>o.parent_qid&&o.parent_qid!=="0")&&s.unsupported.push({code:"limesurvey-subquestions",title:"The LimeSurvey export contains subquestions or matrix rows",detail:"This release imports only standalone List (Radio) and 5 Point Choice questions."}),$(s,{code:"limesurvey-platform-settings-not-imported",title:"LimeSurvey platform and presentation settings are not imported",detail:"Theme, navigation, notification and publication settings remain in LimeSurvey. Review the converted participant presentation."});for(const o of oe){const f=o.qid||o.title||"unknown-question";if(o.mandatory!=="Y"){s.unsupported.push({code:"limesurvey-optional-question",title:`${o.title||f} is not mandatory`,detail:"The participant platform currently requires every imported item."});continue}if(o.other&&o.other!=="N"){s.unsupported.push({code:"limesurvey-other-answer",title:`${o.title||f} allows an “Other” answer`,detail:"Free-text “Other” responses are not imported."});continue}if(o.relevance&&o.relevance!=="1"){s.unsupported.push({code:"limesurvey-question-relevance",title:`${o.title||f} uses relevance logic`,detail:"Conditional questions are not imported."});continue}const E=z(j,"qid",o.qid,"question",h)||o.question,C=z(j,"qid",o.qid,"help",h)||o.help,B=z(j,"qid",o.qid,"script",h)||o.script;if(x(B||"")){s.unsupported.push({code:"limesurvey-question-script",title:`${o.title||f} contains a question script`,detail:"Imported scripts are never executed or silently removed."});continue}if(ve.test(C||"")||_e.test(C||"")||be.test(C||"")){s.unsupported.push({code:"limesurvey-question-help-structure",title:`${o.title||f} contains dynamic or structured help content`,detail:"Executable code, expressions, media and interactive help are not imported."});continue}if(et(C||"")){s.unsupported.push({code:"limesurvey-question-help",title:`${o.title||f} contains participant help text`,detail:"Question help text is not silently merged into or removed from the item wording."});continue}const V=F(E,"Question text",o.title||f,s);if(!V)continue;let R=[],N=[];if(o.type==="5")R=[1,2,3,4,5],N=["1","2","3","4","5"];else if(o.type==="L"){const k=d.filter(b=>b.qid===o.qid&&(!b.scale_id||b.scale_id==="0")).sort((b,J)=>(Number(b.sortorder)||0)-(Number(J.sortorder)||0));if(!tt(k)){s.unsupported.push({code:"limesurvey-answer-order",title:`${o.title||f} has no reliable answer order`,detail:"Every answer needs one unique consecutive sort order starting at 0 or 1."});continue}const Z=k.map(b=>X(b.code)),K=k.map(b=>X(b.assessment_value));if(Z.every(b=>b!==null)&&se(Z))R=Z;else if(K.every(b=>b!==null)&&se(K))R=K,$(s,{code:"limesurvey-assessment-values",title:"LimeSurvey assessment values were used as response values",detail:"Verify every imported value against the LimeSurvey answer table."});else if(nt(k))R=k.map((b,J)=>J+1),$(s,{code:"limesurvey-positional-values",title:"LimeSurvey default answer codes were converted to ordered positions",detail:"The source uses A001 through A00N rather than numeric scores. The converted scale uses positions 1 through N. Confirm the intended values and scoring before conversion."});else{s.unsupported.push({code:"limesurvey-unsupported-values",title:`${o.title||f} has no safe increasing numeric recode`,detail:"Answer codes or assessment values must form one increasing whole-number scale with a constant step."});continue}if(N=k.map((b,J)=>{const qe=z(_,"aid",b.aid,"answer",h)||b.answer;return F(qe,`Answer ${J+1}`,`${o.title||f}/${b.code}`,s)}).filter(b=>!!b),N.length!==k.length||N.length!==R.length)continue}else{s.unsupported.push({code:"limesurvey-unsupported-question",title:`${o.title||f} uses unsupported LimeSurvey type ${o.type||"unknown"}`,detail:"Supported in this release: List (Radio) and 5 Point Choice."});continue}const ae={sourceId:o.title||f,name:o.title||`Item ${D.length+1}`,prompt:V,values:R,labels:N};D.push(ae),s.imported.push(re(ae))}const O=a?.surveyls_description||a?.surveyls_welcometext||"",U=x(O)?F(O,"Survey introduction","surveys_languagesettings",s):null,H=Ae(v,"Imported from LimeSurvey LSS",U??"Answer each imported item about the task that you have just completed.",D,s);return Ie("limesurvey-lss","LimeSurvey LSS",t,v,H,s)}function st(e,t){const n=t.toLowerCase().split(".").at(-1),i=e.trimStart();if(n==="qsf"||i.startsWith("{")&&/"SurveyElements"/.test(e))return"qualtrics-qsf";if(n==="lss"||i.startsWith("<")&&/<LimeSurveyDocType>Survey</.test(e))return"limesurvey-lss";throw new Error("Choose a Qualtrics .qsf file or a LimeSurvey .lss file.")}function rt(e,t,n="auto"){const i=new TextEncoder().encode(e).length;if(!e.trim())throw new Error("The selected questionnaire export is empty.");if(i>He)throw new Error("The selected questionnaire export is larger than the 2 MB review limit.");const s=st(e,t);if(n!=="auto"&&n!==s)throw new Error(`The selected file looks like ${s==="qualtrics-qsf"?"Qualtrics QSF":"LimeSurvey LSS"}, not the chosen format.`);return s==="qualtrics-qsf"?Xe(e,t):it(e,t)}var ot=Object.defineProperty,at=Object.getOwnPropertyDescriptor,p=(e,t,n,i)=>{for(var s=i>1?void 0:i?at(t,n):t,r=e.length-1,g;r>=0;r--)(g=e[r])&&(s=(i?g(t,n,s):g(s))||s);return i&&s&&ot(t,n,s),s};const lt=ne.trim().split(/\r?\n/).filter(Boolean).length,M=ie.match(/var bridgeBuild = '([^']+)'/)?.[1]??"unidentified";function ut(e){const t=Array.isArray(e)?e:[e];return t.length>0&&t.some(n=>{if(!n||typeof n!="object")return!1;const i=n;return"study"in i&&"responses"in i&&"result"in i})}function ct(e){const t="PASTE_THE_GENERATED_PARTICIPANT_PAGE_URL_HERE";if(!e||e.includes(t))throw new Error("A generated participant URL is required for the Qualtrics question HTML.");const n=e.replace(/&/g,"&amp;").replace(/"/g,"&quot;");return Ue.trim().replace(t,n)}function dt(e){const t=e?["Questionnaire:","${e://Field/__js_AQP_INSTRUMENT_NAME}","","${e://Field/__js_AQP_SCORE_NAME}:","${e://Field/__js_AQP_PRIMARY_SCORE}"].join(`
`):"";return je.replace("{{OPTIONAL_SCORE_BLOCK}}",t).replace(/\n{3,}/g,`

`).trim()}let u=class extends Qe{constructor(){super(...arguments),this.instrumentId=Pe,this.customDefinition=null,this.customDraft=pe(),this.customBuilderOpen=!1,this.platformImportSource="auto",this.platformImportReview=null,this.platformImportConfirmed=!1,this.studyId="",this.studyTitle="",this.taskLabel="",this.showScoreToParticipant=!1,this.showSimpleLanguage=!1,this.answerMode="standard",this.largeText=!1,this.audioGuidance=!1,this.recoveryEnabled=!0,this.participantAdjustmentPolicy="participant-choice",this.voiceInputAvailable=!0,this.gazeInputAvailable=!1,this.collectionMode="local",this.qualtricsSurveyUrl="",this.generatedConfig=null,this.participantUrl="",this.message="",this.definitionConfirmation="",this.configurationConfirmation="",this.errorMessage="",this.completedResults=[],this.selectInstrument=e=>{const t=e.currentTarget.value,n=this.availableDefinitions.find(i=>i.id===t)??null;n&&(this.instrumentId=t,n.supports.simplerExplanations||(this.showSimpleLanguage=!1),n.supports.smileyLandmarks||(this.answerMode="standard"),this.generatedConfig=null,this.participantUrl="",this.definitionConfirmation=`${n.name} ${n.version} selected. Complete the study details, then generate a new configuration.`,this.configurationConfirmation="",this.message=`${n.name} selected. Generate a new configuration before testing.`)},this.importPlatformQuestionnaire=async e=>{const t=e.currentTarget,n=t.files?.[0];if(n){this.errorMessage="",this.definitionConfirmation="",this.platformImportReview=null,this.platformImportConfirmed=!1;try{const i=rt(await n.text(),n.name,this.platformImportSource);this.platformImportReview=i,i.draft&&(this.customDraft=structuredClone(i.draft)),this.message=i.canConvert?"Import review ready. Check every section and confirm scoring before conversion.":"Import review found unsupported content. No partial questionnaire was created.",this.revealConductorResult("#platform-import-review")}catch(i){this.showError(i instanceof Error?i.message:"The questionnaire export could not be reviewed.")}finally{t.value=""}}},this.usePlatformImport=()=>{const e=this.platformImportReview;if(!(!e?.canConvert||!e.draft||!this.platformImportConfirmed)){this.errorMessage="",this.definitionConfirmation="";try{const t=me(this.customDraft);this.activateCustomDefinition(t,"imported")}catch(t){this.showError(t instanceof Error?t.message:"The reviewed questionnaire could not be converted.")}}},this.addCustomItem=()=>{this.customDraft.items.length>=L||(this.customDraft={...this.customDraft,items:[...this.customDraft.items,Y({name:`Item ${this.customDraft.items.length+1}`})]})},this.useCustomDraft=()=>{this.errorMessage="",this.definitionConfirmation="";try{this.activateCustomDefinition(me(this.customDraft),"validated")}catch(e){this.showError(e instanceof Error?e.message:"The custom questionnaire could not be validated.")}},this.importCustomDefinition=async e=>{const t=e.currentTarget,n=t.files?.[0];if(n){this.errorMessage="",this.definitionConfirmation="";try{const i=ye(JSON.parse(await n.text()));this.activateCustomDefinition(i,"imported")}catch(i){this.showError(i instanceof Error?i.message:"The questionnaire definition file could not be read.")}finally{t.value=""}}},this.downloadCustomDefinition=()=>{this.customDefinition&&W(Ge(this.customDefinition),JSON.stringify(this.customDefinition,null,2),"application/json")},this.resetCustomDraft=()=>{this.customDraft=pe(),this.platformImportReview=null,this.platformImportConfirmed=!1,this.message="Custom questionnaire builder fields reset. The selected questionnaire is unchanged until you validate a new definition."},this.generateParticipantLink=()=>{this.errorMessage="",this.configurationConfirmation="";try{const e=Re({instrumentId:this.instrumentId,...this.customDefinition?.id===this.instrumentId?{questionnaireDefinition:this.customDefinition}:{},studyId:this.studyId,studyTitle:this.studyTitle,taskLabel:this.taskLabel,showScoreToParticipant:this.showScoreToParticipant,support:this.currentSupportConfig(),collection:this.currentCollectionConfig()});this.useConfiguration(e),this.configurationConfirmation="Participant link and configuration generated. The ready-to-use files and link are shown below.",this.message="",this.revealConductorResult("#configuration-ready-panel")}catch(e){this.showError(e instanceof Error?e.message:"The study configuration could not be generated.")}},this.copyParticipantLink=async()=>{this.participantUrl&&await this.copySetupAsset(this.participantUrl,"participant link")},this.copySetupAsset=async(e,t)=>{try{if(!navigator.clipboard?.writeText)throw new Error("Clipboard API unavailable.");await navigator.clipboard.writeText(e),this.message=`${t.charAt(0).toUpperCase()}${t.slice(1)} copied.`}catch{this.message=`Automatic copy was unavailable. Select and copy the ${t} from its text box.`}},this.downloadConfiguration=()=>{this.generatedConfig&&W(`${this.generatedConfig.studyId}-${this.generatedConfig.configId}.json`,JSON.stringify(this.generatedConfig,null,2),"application/json")},this.importConfiguration=async e=>{const t=e.currentTarget,n=t.files?.[0];if(n){this.errorMessage="",this.configurationConfirmation="";try{const i=JSON.parse(await n.text()),s=xe(i);if(!s)throw ut(i)?new Error("This is a completed result file, not a study configuration. Import the JSON downloaded from Configuration ready."):new Error("This is not a valid Version 0.8 study configuration or supported Version 0.7 configuration.");this.useConfiguration(s),this.configurationConfirmation="Configuration imported and participant link reproduced. The configuration ID and files are shown below.",this.message="",this.revealConductorResult("#configuration-ready-panel")}catch(i){this.showError(i instanceof Error?i.message:"The configuration file could not be read.")}finally{t.value=""}}},this.refreshResults=()=>{this.completedResults=Ne()},this.exportResultsJson=()=>{this.completedResults.length&&W(`accessible-questionnaire-results-${new Date().toISOString().slice(0,10)}.json`,JSON.stringify(this.completedResults,null,2),"application/json")},this.exportResultsCsv=()=>{this.completedResults.length&&W(`accessible-questionnaire-results-${new Date().toISOString().slice(0,10)}.csv`,`\uFEFF${De(this.completedResults)}`,"text/csv")},this.eraseResults=()=>{window.confirm("Erase every completed questionnaire record stored by this site in this browser? Confirm only after checking the exported files.")&&(ke(),this.refreshResults(),this.message="Local completed records erased.")}}connectedCallback(){super.connectedCallback(),this.refreshResults(),window.addEventListener("storage",this.refreshResults)}disconnectedCallback(){window.removeEventListener("storage",this.refreshResults),super.disconnectedCallback()}createRenderRoot(){return this}get definition(){return Le(this.instrumentId,this.customDefinition??void 0)}get availableDefinitions(){return this.customDefinition?[...ue,this.customDefinition]:ue}render(){return l`
      <a class="skip-link" href="#conductor-main">Skip to study setup</a>
      <main class="app-shell conductor-shell" id="conductor-main">
        <header class="app-header">
          <p class="eyebrow">
            Study conductor · Version ${ee} · Qualtrics package ${M}
          </p>
        ...
        <aside class="boundary-note important-boundary" aria-labelledby="current-generator-heading">
          <h2 id="current-generator-heading">Current Qualtrics generator: ${M}</h2>
          <p>
            Every generated JavaScript block must contain
            <code>var bridgeBuild = '${M}';</code>. If it shows another value, that browser tab is
            running a stale conductor build. Close that tab and reopen the versioned
            <a href="study.html?package=${M}">Prepare a study page</a> before copying anything.
          </p>
        </aside>
        ...
        <section class="panel conductor-panel" aria-labelledby="study-details-heading">
          ...
          <div class="form-grid">
            ...
            <aside
              ...
              tabindex="-1"
              aria-describedby=${this.definitionConfirmation?"definition-confirmation-message":T}
            >
              ...
              <span>
                ...
                ${te(this.definition).length}
                ${this.definition.scale.type.replace("-"," ")} response values,
                ${Oe(this.definition).length} comparisons,
            ...
            ${this.customBuilderOpen?this.renderCustomQuestionnaireBuilder():T}
            <label>
        ...
        <section class="panel conductor-panel" aria-labelledby="support-config-heading">
          ...
          ${this.definition.supports.smileyLandmarks?l`<fieldset class="answer-mode-control conductor-answer-mode">
                ...
                <label>
                  ...
                  <span>
                    <strong>Standard ${te(this.definition).length}-value scale</strong>
                ...
                ${this.definition.shortName} uses its standard ${te(this.definition).length}-value
        ...
        <section class="panel conductor-panel" aria-labelledby="link-heading">
          ...
          ${this.generatedConfig?l`<div
                ...
                aria-describedby=${this.configurationConfirmation?"configuration-confirmation-message":T}
                tabindex="-1"
              >
                ...
                <div class="button-row compact">
                  ...
                  ${this.generatedConfig.collection.mode==="local"?l`<a class="secondary-button link-button" href=${this.participantUrl} target="_blank" rel="noopener">Open participant page</a>`:T}
                  <button class="secondary-button" type="button" @click=${this.downloadConfiguration}>Download configuration JSON</button>
                </div>
                ${this.generatedConfig.collection.mode==="qualtrics"?this.renderQualtricsSetup():T}
                <p class="support-boundary">
        ...
        <section class="panel conductor-panel" aria-labelledby="remote-heading">
          ...
          <p>
            Version ${ee} includes a Qualtrics parent bridge. The participant page sends a complete record only to the
      ...
      >
        ...
        >
          ...
          <p>
            ...
            1–${L} required single-choice items that
        ...
        <fieldset class="custom-items">
          ...
          <button
            class="secondary-button"
            type="button"
            ?disabled=${this.customDraft.items.length>=L}
    ...
    `}renderPlatformQuestionnaireImport(){const e=this.platformImportReview;return l`
      ...
      >
        ...
        ${e?this.renderPlatformQuestionnaireReview(e):T}
      </section>
    ...
    `}updateCustomDraft(e,t){this.customDraft={...this.customDraft,[e]:t},this.platformImportReview&&(this.platformImportConfirmed=!1)}updateCustomItem(e,t,n){this.customDraft={...this.customDraft,items:this.customDraft.items.map((i,s)=>s===e?{...i,[t]:n}:i)},this.platformImportReview&&(this.platformImportConfirmed=!1)}removeCustomItem(e){this.customDraft.items.length!==1&&(this.customDraft={...this.customDraft,items:this.customDraft.items.filter((t,n)=>n!==e)})}activateCustomDefinition(e,t){this.customDefinition=e,this.instrumentId=e.id,e.supports.simplerExplanations||(this.showSimpleLanguage=!1),this.answerMode="standard",this.generatedConfig=null,this.participantUrl="";const n=t==="imported"?"imported, validated and selected":"validated and selected";this.definitionConfirmation=`${e.name} ${e.version} ${n}. Complete the study details, then generate a new configuration.`,this.configurationConfirmation="",this.message="",this.revealConductorResult("#selected-questionnaire-summary")}booleanOption(e,t,n,i=""){return l`<label class="toggle-card conductor-toggle">
      ...
      <div class="qualtrics-setup" role="region" aria-labelledby="qualtrics-setup-heading">
        ...
        <p>
          ...
          platform ${ee}; Qualtrics bridge ${M}.
        ...
        <aside class="boundary-note important-boundary">
          <p>
            ...
            ${M} and say that diagnostic fields were staged. Then complete one
        ...
        <ol class="qualtrics-install-steps">
          ...
          <li>
            ...
            <button
              class="secondary-button"
              type="button"
              @click=${()=>this.copySetupAsset(ne.trim(),"Embedded Data field list")}
            >
    ...
    `}showError(e){this.errorMessage=e,this.updateComplete.then(()=>{const t=this.querySelector("#conductor-error");t&&ce(t)})}revealConductorResult(e){this.updateComplete.then(()=>{const t=this.querySelector(e);t&&ce(t,{block:"start"})})}};p([m()],u.prototype,"instrumentId",2);p([m()],u.prototype,"customDefinition",2);p([m()],u.prototype,"customDraft",2);p([m()],u.prototype,"customBuilderOpen",2);p([m()],u.prototype,"platformImportSource",2);p([m()],u.prototype,"platformImportReview",2);p([m()],u.prototype,"platformImportConfirmed",2);p([m()],u.prototype,"studyId",2);p([m()],u.prototype,"studyTitle",2);p([m()],u.prototype,"taskLabel",2);p([m()],u.prototype,"showScoreToParticipant",2);p([m()],u.prototype,"showSimpleLanguage",2);p([m()],u.prototype,"answerMode",2);p([m()],u.prototype,"largeText",2);p([m()],u.prototype,"audioGuidance",2);p([m()],u.prototype,"recoveryEnabled",2);p([m()],u.prototype,"participantAdjustmentPolicy",2);p([m()],u.prototype,"voiceInputAvailable",2);p([m()],u.prototype,"gazeInputAvailable",2);p([m()],u.prototype,"collectionMode",2);p([m()],u.prototype,"qualtricsSurveyUrl",2);p([m()],u.prototype,"generatedConfig",2);p([m()],u.prototype,"participantUrl",2);p([m()],u.prototype,"message",2);p([m()],u.prototype,"definitionConfirmation",2);p([m()],u.prototype,"configurationConfirmation",2);p([m()],u.prototype,"errorMessage",2);p([m()],u.prototype,"completedResults",2);u=p([Ee("study-conductor-app")],u);
